...
G_DEFINE_FINAL_TYPE (GxDataGraph, gx_data_graph, GTK_TYPE_WIDGET)
...
static void
refresh_path (GxDataGraph *self,
              double       width,
              double       height);

static void
animate_lower_bound (GxDataGraph *self,
                     double       from,
                     double       to);

static void
animate_upper_bound (GxDataGraph *self,
                     double       from,
                     double       to);

static void
gx_data_graph_dispose (GObject *object)
{
  GxDataGraph *self = GX_DATA_GRAPH (object);
  ...
  G_OBJECT_CLASS (gx_data_graph_parent_class)->dispose (object);
}

static void
gx_data_graph_get_property (GObject    *object,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MODEL:
      g_value_set_object (value, gx_data_graph_get_model (self));
      break;
    case PROP_INDEPENDENT_AXIS_LABEL:
      g_value_set_string (value, gx_data_graph_get_independent_axis_label (self));
      break;
    case PROP_DEPENDENT_AXIS_LABEL:
      g_value_set_string (value, gx_data_graph_get_dependent_axis_label (self));
      break;
    case PROP_INDEPENDENT_DECIMALS:
      g_value_set_int (value, gx_data_graph_get_independent_decimals (self));
      break;
    case PROP_DEPENDENT_DECIMALS:
      g_value_set_int (value, gx_data_graph_get_dependent_decimals (self));
      break;
    case PROP_HAS_DEPENDENT_MIN:
      g_value_set_boolean (value, gx_data_graph_get_has_dependent_min (self));
      break;
    case PROP_DEPENDENT_MIN:
      g_value_set_double (value, gx_data_graph_get_dependent_min (self));
      break;
    case PROP_HAS_DEPENDENT_MAX:
      g_value_set_boolean (value, gx_data_graph_get_has_dependent_max (self));
      break;
    case PROP_DEPENDENT_MAX:
      g_value_set_double (value, gx_data_graph_get_dependent_max (self));
      break;
    case PROP_TRANSITION_PROGRESS:
      g_value_set_double (value, gx_data_graph_get_transition_progress (self));
      break;
    default:
...
static void
gx_data_graph_set_property (GObject      *object,
                            guint         prop_id,
                            const GValue *value,
                            GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MODEL:
      gx_data_graph_set_model (self, g_value_get_object (value));
      break;
    case PROP_INDEPENDENT_AXIS_LABEL:
      gx_data_graph_set_independent_axis_label (self, g_value_get_string (value));
      break;
    case PROP_DEPENDENT_AXIS_LABEL:
      gx_data_graph_set_dependent_axis_label (self, g_value_get_string (value));
      break;
    case PROP_INDEPENDENT_DECIMALS:
      gx_data_graph_set_independent_decimals (self, g_value_get_int (value));
      break;
    case PROP_DEPENDENT_DECIMALS:
      gx_data_graph_set_dependent_decimals (self, g_value_get_int (value));
      break;
    case PROP_HAS_DEPENDENT_MIN:
      gx_data_graph_set_has_dependent_min (self, g_value_get_boolean (value));
      break;
    case PROP_DEPENDENT_MIN:
      gx_data_graph_set_dependent_min (self, g_value_get_double (value));
      break;
    case PROP_HAS_DEPENDENT_MAX:
      gx_data_graph_set_has_dependent_max (self, g_value_get_boolean (value));
      break;
    case PROP_DEPENDENT_MAX:
      gx_data_graph_set_dependent_max (self, g_value_get_double (value));
      break;
    case PROP_TRANSITION_PROGRESS:
      gx_data_graph_set_transition_progress (self, g_value_get_double (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
gx_data_graph_size_allocate (GtkWidget *widget,
                             int        width,
                             int        height,
                             int        baseline)
{
  GxDataGraph *self = GX_DATA_GRAPH (widget);
...
static void
gx_data_graph_snapshot (GtkWidget   *widget,
...
static void
gx_data_graph_class_init (GxDataGraphClass *klass)
{
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose      = gx_data_graph_dispose;
  object_class->get_property = gx_data_graph_get_property;
  object_class->set_property = gx_data_graph_set_property;
  ...
  widget_class->size_allocate = gx_data_graph_size_allocate;
  widget_class->snapshot      = gx_data_graph_snapshot;
}

static void
gx_data_graph_init (GxDataGraph *self)
{
...
{
  return g_object_new (GX_TYPE_DATA_GRAPH, NULL);
}

GListModel *
gx_data_graph_get_model (GxDataGraph *self)
{
  g_return_val_if_fail (GX_IS_DATA_GRAPH (self), NULL);
  return self->model;
}
...
{
  g_return_val_if_fail (GX_IS_DATA_GRAPH (self), 0);
...
{
  g_return_val_if_fail (GX_IS_DATA_GRAPH (self), FALSE);
...
{
  g_return_val_if_fail (GX_IS_DATA_GRAPH (self), 0.0);
...
gx_data_graph_set_model (GxDataGraph *self,
                         GListModel  *model)
{
  g_return_if_fail (GX_IS_DATA_GRAPH (self));
