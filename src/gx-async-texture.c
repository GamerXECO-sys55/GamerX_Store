...
#define G_LOG_DOMAIN "GAMERX_STORE::ASYNC-TEXTURE"
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (source_uri, g_free);
    GX_RELEASE_DATA (cache_into, g_object_unref);
    GX_RELEASE_DATA (cache_into_path, g_free);
    GX_RELEASE_DATA (cancellable, g_object_unref));

struct _GxAsyncTexture
{
...
static void
maybe_load (GxAsyncTexture *self);
...
static gboolean
idle_notify (GxAsyncTexture *self);

static void
gx_async_texture_dispose (GObject *object)
{
  GxAsyncTexture *self = GX_ASYNC_TEXTURE (object);
  ...
  G_OBJECT_CLASS (gx_async_texture_parent_class)->dispose (object);
}

static void
gx_async_texture_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SOURCE:
      g_value_set_object (value, gx_async_texture_get_source (self));
      break;
    case PROP_CACHE_INTO:
      g_value_set_object (value, gx_async_texture_get_cache_into (self));
      break;
    case PROP_LOADED:
      g_value_set_boolean (value, gx_async_texture_get_loaded (self));
      break;
    default:
...
static void
gx_async_texture_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
...
static void
gx_async_texture_class_init (GxAsyncTextureClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose      = gx_async_texture_dispose;
  object_class->get_property = gx_async_texture_get_property;
  object_class->set_property = gx_async_texture_set_property;
...
static void
gx_async_texture_init (GxAsyncTexture *self)
{
  self->retries   = 0;
  self->paintable = NULL;
...
paintable_snapshot (GdkPaintable *paintable,
                    ...
                    double        height)
{
  GxAsyncTexture *self            = GX_ASYNC_TEXTURE (paintable);
...
{
  g_return_val_if_fail (GX_IS_ASYNC_TEXTURE (self), FALSE);
  return self->source;
}
...
gx_async_texture_dup_texture (GxAsyncTexture *self)
{
  ...
  g_return_val_if_fail (GX_IS_ASYNC_TEXTURE (self), NULL);
...
gx_async_texture_ensure (GxAsyncTexture *self)
{
  ...
  g_return_if_fail (GX_IS_ASYNC_TEXTURE (self));
...
load_fiber_work (LoadData *data)
{
  ...
  static GxGuard *gates[MAX_CONCURRENT_LOADS]          = { 0 };
      ...
      if (is_http)
        {
          ...
          result = dex_await (
              dex_future_first (
                  gx_download_worker_invoke (
                      gx_download_worker_get_default (),
                      source, load_file),
