...
G_DEFINE_FINAL_TYPE (GxPatternedBackground, gx_patterned_background, GTK_TYPE_WIDGET);
...
static void
gx_patterned_background_dispose (GObject *object)
{
  GxPatternedBackground *self = GX_PATTERNED_BACKGROUND (object);

  g_clear_pointer (&self->widget, gtk_widget_unparent);
  g_clear_pointer (&self->tint, g_free);

  G_OBJECT_CLASS (gx_patterned_background_parent_class)->dispose (object);
}

static void
gx_patterned_background_get_property (GObject    *object,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_WIDGET:
      g_value_set_object (value, gx_patterned_background_get_widget (self));
      break;
    case PROP_TINT:
      g_value_set_string (value, gx_patterned_background_get_tint (self));
      break;
    default:
...
static void
gx_patterned_background_set_property (GObject      *object,
                                      guint         prop_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_WIDGET:
      gx_patterned_background_set_widget (self, g_value_get_object (value));
      break;
    case PROP_TINT:
      gx_patterned_background_set_tint (self, g_value_get_string (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
gx_patterned_background_size_allocate (GtkWidget *widget,
                                       int        width,
                                       int        height,
                                       int        baseline)
{
  GxPatternedBackground *self = GX_PATTERNED_BACKGROUND (widget);
...
static void
gx_patterned_background_snapshot (GtkWidget   *widget,
...
static void
gx_patterned_background_class_init (GxPatternedBackgroundClass *klass)
{
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->set_property = gx_patterned_background_set_property;
  object_class->get_property = gx_patterned_background_get_property;
  object_class->dispose      = gx_patterned_background_dispose;
  ...
  widget_class->size_allocate = gx_patterned_background_size_allocate;
  widget_class->snapshot      = gx_patterned_background_snapshot;
}

static void
gx_patterned_background_init (GxPatternedBackground *self)
{
}
...
{
  return g_object_new (GX_TYPE_PATTERNED_BACKGROUND, NULL);
}

GtkWidget *
gx_patterned_background_get_widget (GxPatternedBackground *self)
{
  g_return_val_if_fail (GX_IS_PATTERNED_BACKGROUND (self), NULL);
  return self->widget;
}
...
gx_patterned_background_set_widget (GxPatternedBackground *self,
                                    GtkWidget             *widget)
{
  g_return_if_fail (GX_IS_PATTERNED_BACKGROUND (self));
