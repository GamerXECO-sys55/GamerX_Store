...
G_DEFINE_FINAL_TYPE (GxShareDialog, gx_share_dialog, ADW_TYPE_DIALOG)
...
static void
follow_link_cb (GxShareDialog *self,
                GtkButton     *button)
{
  const char *link = NULL;
...
static AdwActionRow *
create_url_action_row (GxShareDialog *self, GxUrl *url_item)
{
...
static void
populate_urls (GxShareDialog *self)
{
...
static void
gx_share_dialog_dispose (GObject *object)
{
  GxShareDialog *self = GX_SHARE_DIALOG (object);

  g_clear_object (&self->entry);

  G_OBJECT_CLASS (gx_share_dialog_parent_class)->dispose (object);
}

static void
gx_share_dialog_get_property (GObject    *object,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
...
static void
gx_share_dialog_set_property (GObject      *object,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
...
static void
gx_share_dialog_class_init (GxShareDialogClass *klass)
{
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose      = gx_share_dialog_dispose;
  object_class->get_property = gx_share_dialog_get_property;
  object_class->set_property = gx_share_dialog_set_property;
...
static void
gx_share_dialog_init (GxShareDialog *self)
{
  gtk_widget_init_template (GTK_WIDGET (self));
}

AdwDialog *
gx_share_dialog_new (GxEntry *entry)
{
  GxShareDialog *share_dialog = NULL;

  share_dialog = g_object_new (
      GX_TYPE_SHARE_DIALOG,
      "entry", entry,
      NULL);
