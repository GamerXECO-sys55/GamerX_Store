  ...
  if (g_once_init_enter (&stack_size))
    {
      ...
      envvar = g_getenv ("GAMERX_STORE_DEX_STACK_SIZE");
      if (envvar != NULL)
        {
          ...
          if (variant != NULL)
            {
              ...
              if (parse_result < dex_get_min_stack_size ())
                g_critical ("GAMERX_STORE_DEX_STACK_SIZE must be greater than %zu on this system",
                            dex_get_min_stack_size ());
              else
                value = parse_result;
            }
          else
            g_critical ("GAMERX_STORE_DEX_STACK_SIZE is invalid: %s", local_error->message);
        }
