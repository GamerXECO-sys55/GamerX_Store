...
#define G_LOG_DOMAIN "GAMERX_STORE::TRANSACTIONS"
...
G_DEFINE_FINAL_TYPE (GxTransactionManager, gx_transaction_manager, G_TYPE_OBJECT)
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (backend, g_object_unref);
    GX_RELEASE_DATA (transaction, gx_transaction_dismiss);
    GX_RELEASE_DATA (channel, dex_unref);
    GX_RELEASE_DATA (timer, g_timer_destroy);
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (id, g_free);
    GX_RELEASE_DATA (dialog, g_object_unref));

static DexFuture *
transaction_fiber (QueuedScheduleData *data);

static int
execute_hook (GxTransactionManager *self,
              GHashTable           *hook,
              const char           *ts_kind,
              const char           *ts_appid);
...
static void
dispatch_next (GxTransactionManager *self);

static void
gx_transaction_manager_dispose (GObject *object)
{
  GxTransactionManager *self = GX_TRANSACTION_MANAGER (object);
  ...
  G_OBJECT_CLASS (gx_transaction_manager_parent_class)->dispose (object);
}

static void
gx_transaction_manager_get_property (GObject    *object,
                                     guint       prop_id,
                                     GValue     *value,
                                     GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CONFIG:
      g_value_set_boxed (value, gx_transaction_manager_get_config (self));
      break;
    case PROP_BACKEND:
      g_value_set_object (value, gx_transaction_manager_get_backend (self));
      break;
    case PROP_PAUSED:
      g_value_set_boolean (value, gx_transaction_manager_get_paused (self));
      break;
    case PROP_TRANSACTIONS:
      g_value_set_object (value, self->transactions);
      break;
    case PROP_HAS_TRANSACTIONS:
      g_value_set_boolean (value, gx_transaction_manager_get_has_transactions (self));
      break;
    case PROP_ACTIVE:
      g_value_set_boolean (value, gx_transaction_manager_get_active (self));
      break;
...
static void
gx_transaction_manager_set_property (GObject      *object,
                                     guint         prop_id,
                                     const GValue *value,
                                     GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CONFIG:
      gx_transaction_manager_set_config (self, g_value_get_boxed (value));
      break;
    case PROP_BACKEND:
      gx_transaction_manager_set_backend (self, g_value_get_object (value));
      break;
    case PROP_PAUSED:
      gx_transaction_manager_set_paused (self, g_value_get_boolean (value));
      break;
    case PROP_TRANSACTIONS:
    case PROP_HAS_TRANSACTIONS:
    case PROP_ACTIVE:
...
static void
gx_transaction_manager_class_init (GxTransactionManagerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose      = gx_transaction_manager_dispose;
  object_class->get_property = gx_transaction_manager_get_property;
  object_class->set_property = gx_transaction_manager_set_property;
  ...
  g_type_ensure (GX_TYPE_TRANSACTION_VIEW);
  ...
  signals[SIGNAL_SUCCESS] =
      g_signal_new (
          ...
          0,
          NULL, NULL,
          gx_marshal_VOID__OBJECT_BOXED,
          G_TYPE_NONE,
          2, GX_TYPE_TRANSACTION, G_TYPE_HASH_TABLE, 0);
  g_signal_set_va_marshaller (
      signals[SIGNAL_SUCCESS],
      G_TYPE_FROM_CLASS (klass),
      gx_marshal_VOID__OBJECT_BOXEDv);
...
static void
gx_transaction_manager_init (GxTransactionManager *self)
{
  self->transactions = g_list_store_new (GX_TYPE_TRANSACTION);
...
{
  return g_object_new (GX_TYPE_TRANSACTION_MANAGER, NULL);
}
...
gx_transaction_manager_set_config (GxTransactionManager *self,
                                   GHashTable           *config)
{
  g_return_if_fail (GX_IS_TRANSACTION_MANAGER (self));
...
{
  g_return_val_if_fail (GX_IS_TRANSACTION_MANAGER (self), NULL);
  return self->config;
}
...
{
  g_return_if_fail (GX_IS_TRANSACTION_MANAGER (self));
  g_return_if_fail (backend == NULL || GX_IS_BACKEND (backend));
...
{
  g_return_val_if_fail (GX_IS_TRANSACTION_MANAGER (self), FALSE);
  return self->paused;
}
                            ...
                            GxTransaction        *transaction)
{
  ...
  g_return_if_fail (GX_IS_TRANSACTION (transaction));

  gx_transaction_hold (transaction);

  if (self->queue.length > 0)
    {
      GxTransaction *to_merge[2] = { 0 };
      guint          position    = 0;
      ...
      transaction = gx_transaction_new_merged (to_merge, G_N_ELEMENTS (to_merge));
...
{
  dex_return_error_if_fail (GX_IS_TRANSACTION_MANAGER (self));
...
transaction_fiber (QueuedScheduleData *data)
{
  GxTransactionManager *self        = data->self;
  GxBackend            *backend     = data->backend;
  GxTransaction        *transaction = data->transaction;
...
#define COUNT(type)                                  \
  ...
  {                                                  \
    GListModel *model = NULL;                        \
                                                     \
    model = gx_transaction_get_##type (transaction); \
    if (model != NULL)                               \
          ...
          if (i < n_installs)
            {
              ...
              entry   = g_list_model_get_item (
                  gx_transaction_get_installs (transaction),
                  i);
            }
          else if (i < n_installs + n_updates)
            {
              ts_kind = "update";
              entry   = g_list_model_get_item (
                  gx_transaction_get_updates (transaction),
                  i - n_installs);
            }
          else
            {
              ts_kind = "removal";
              entry   = g_list_model_get_item (
                  gx_transaction_get_removals (transaction),
                  i - n_updates - n_installs);
            }
          ts_appid = gx_entry_get_id (entry);
          ...
          for (guint k = 0; k < hooks->len; k++)
            {
              ...
              if (hook_result == HOOK_CONFIRM ||
                ...
                return dex_future_new_reject (
                    GX_TRANSACTION_MGR_ERROR,
                    GX_TRANSACTION_MGR_ERROR_CANCELLED_BY_HOOK,
                    "The transaction was prevented by a configured hook");
            }
        }
    }

  store = g_list_store_new (GX_TYPE_TRANSACTION);
  g_list_store_append (store, transaction);

  future = gx_backend_merge_and_schedule_transactions (
      backend,
      G_LIST_MODEL (store),
      channel,
      cancellable);
  ...
  for (;;)
    {
      ...
      if (GX_IS_BACKEND_TRANSACTION_OP_PAYLOAD (object))
        {
          if (g_hash_table_contains (op_set, object))
            {
              ...
              if (error != NULL)
                gx_transaction_error_out_task (
                    transaction, GX_BACKEND_TRANSACTION_OP_PAYLOAD (object), error);
              else
                gx_transaction_finish_task (
                    transaction, GX_BACKEND_TRANSACTION_OP_PAYLOAD (object));
              g_hash_table_remove (op_set, object);
            }
          else
            {
              gx_transaction_add_task (
                  transaction, GX_BACKEND_TRANSACTION_OP_PAYLOAD (object));
              g_hash_table_add (op_set, g_object_ref (object));
            }
        }
      else if (GX_IS_BACKEND_TRANSACTION_OP_PROGRESS_PAYLOAD (object))
        {
          const char *status         = NULL;
          gboolean    is_estimating  = FALSE;
          double      total_progress = 0.0;

          gx_transaction_update_task (
              transaction, GX_BACKEND_TRANSACTION_OP_PROGRESS_PAYLOAD (object));

          status = gx_backend_transaction_op_progress_payload_get_status (
              GX_BACKEND_TRANSACTION_OP_PROGRESS_PAYLOAD (object));
          is_estimating = gx_backend_transaction_op_progress_payload_get_is_estimating (
              GX_BACKEND_TRANSACTION_OP_PROGRESS_PAYLOAD (object));
          total_progress = gx_backend_transaction_op_progress_payload_get_total_progress (
              GX_BACKEND_TRANSACTION_OP_PROGRESS_PAYLOAD (object));
  ...
  for (guint stage = 0;; stage++)
    {
      ...
      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_INITIATED_UNIX_STAMP", timestamp_sec, TRUE);
      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_INITIATED_UNIX_STAMP_USEC", timestamp_usec, TRUE);

      stage_str = g_strdup_printf ("%d", stage);
      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_STAGE_IDX", stage_str, TRUE);

      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_ID", id, TRUE);
      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_TYPE", type, TRUE);

      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_WAS_ABORTED", hook_aborted ? "true" : "false", TRUE);

      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_TS_APPID", ts_appid, TRUE);
      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_TS_TYPE", ts_type, TRUE);
      ...
      if (finish)
      ...
      else if (current_dialog != NULL)
        {
          ...
          if (window != NULL)
            {
              ...
              response = dex_await_string (
                  gx_make_alert_dialog_future (ADW_ALERT_DIALOG (current_dialog->dialog)),
                  &local_error);
              if (response == NULL)
          ...
          g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_DIALOG_ID", current_dialog->id, TRUE);
          g_subprocess_launcher_setenv (
              launcher,
              "GAMERX_STORE_HOOK_DIALOG_RESPONSE_ID",
              response != NULL
                  ? response
                  : adw_alert_dialog_get_default_response (
                        ADW_ALERT_DIALOG (current_dialog->dialog)),
        ...
        hook_stage = "action";
      g_subprocess_launcher_setenv (launcher, "GAMERX_STORE_HOOK_STAGE", hook_stage, TRUE);
