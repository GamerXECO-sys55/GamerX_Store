...
G_DEFINE_FINAL_TYPE (GxGroupTileCssWatcher, gx_group_tile_css_watcher, G_TYPE_OBJECT);
...
static void
refresh (GxGroupTileCssWatcher *self);

static void
clear (GxGroupTileCssWatcher *self);

static void
gx_group_tile_css_watcher_dispose (GObject *object)
{
  GxGroupTileCssWatcher *self = GX_GROUP_TILE_CSS_WATCHER (object);
  ...
  G_OBJECT_CLASS (gx_group_tile_css_watcher_parent_class)->dispose (object);
}

static void
gx_group_tile_css_watcher_get_property (GObject    *object,
                                        guint       prop_id,
                                        GValue     *value,
                                        GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_WIDGET:
      g_value_take_object (value, gx_group_tile_css_watcher_dup_widget (self));
      break;
    case PROP_GROUP:
      g_value_set_object (value, gx_group_tile_css_watcher_get_group (self));
      break;
    default:
...
static void
gx_group_tile_css_watcher_set_property (GObject      *object,
                                        guint         prop_id,
                                        const GValue *value,
                                        GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_WIDGET:
      gx_group_tile_css_watcher_set_widget (self, g_value_get_object (value));
      break;
    case PROP_GROUP:
      gx_group_tile_css_watcher_set_group (self, g_value_get_object (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
gx_group_tile_css_watcher_class_init (GxGroupTileCssWatcherClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_group_tile_css_watcher_set_property;
  object_class->get_property = gx_group_tile_css_watcher_get_property;
  object_class->dispose      = gx_group_tile_css_watcher_dispose;
...
static void
dark_changed (GxGroupTileCssWatcher *self,
              GParamSpec            *pspec,
              AdwStyleManager       *mgr)
{
...
static void
gx_group_tile_css_watcher_init (GxGroupTileCssWatcher *self)
{
...
{
  return g_object_new (GX_TYPE_GROUP_TILE_CSS_WATCHER, NULL);
}

GtkWidget *
gx_group_tile_css_watcher_dup_widget (GxGroupTileCssWatcher *self)
{
  g_return_val_if_fail (GX_IS_GROUP_TILE_CSS_WATCHER (self), NULL);
...
gx_group_tile_css_watcher_set_widget (GxGroupTileCssWatcher *self,
                                      GtkWidget             *widget)
{
  g_return_if_fail (GX_IS_GROUP_TILE_CSS_WATCHER (self));
...
refresh (GxGroupTileCssWatcher *self)
{
  ...
  id                 = gx_entry_group_get_id (self->group);
  light_accent_color = gx_entry_group_get_light_accent_color (self->group);
  dark_accent_color  = gx_entry_group_get_dark_accent_color (self->group);
