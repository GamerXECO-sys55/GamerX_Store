...
G_DEFINE_FINAL_TYPE_WITH_CODE (
    GxInhibitedScrollable,
    gx_inhibited_scrollable,
    GTK_TYPE_WIDGET,
...
static void
child_adjustment_value_changed (GxInhibitedScrollable *self,
                                GtkAdjustment         *adjustment);

static void
setup_hadjustments (GxInhibitedScrollable *self);

static void
setup_vadjustments (GxInhibitedScrollable *self);

static void
clear_bindings (GxInhibitedScrollable *self);

static void
gx_inhibited_scrollable_dispose (GObject *object)
{
  GxInhibitedScrollable *self = GX_INHIBITED_SCROLLABLE (object);
  ...
  G_OBJECT_CLASS (gx_inhibited_scrollable_parent_class)->dispose (object);
}

static void
gx_inhibited_scrollable_get_property (GObject    *object,
                                      guint       prop_id,
                                      GValue     *value,
                                      GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_SCROLLABLE:
      g_value_set_object (value, gx_inhibited_scrollable_get_scrollable (self));
      break;
...
static void
gx_inhibited_scrollable_set_property (GObject      *object,
                                      guint         prop_id,
                                      const GValue *value,
                                      GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SCROLLABLE:
      gx_inhibited_scrollable_set_scrollable (self, g_value_get_object (value));
      break;
    case PROP_HADJUSTMENT:
      g_clear_object (&self->hadjustment);
...
static void
gx_inhibited_scrollable_size_allocate (GtkWidget *widget,
                                       int        width,
                                       int        height,
                                       int        baseline)
{
  GxInhibitedScrollable *self = GX_INHIBITED_SCROLLABLE (widget);
...
static void
gx_inhibited_scrollable_class_init (GxInhibitedScrollableClass *klass)
{
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose      = gx_inhibited_scrollable_dispose;
  object_class->get_property = gx_inhibited_scrollable_get_property;
  object_class->set_property = gx_inhibited_scrollable_set_property;
  ...
  widget_class->size_allocate = gx_inhibited_scrollable_size_allocate;
}

static void
gx_inhibited_scrollable_init (GxInhibitedScrollable *self)
{
}
...
scrollable_get_border (GtkScrollable *scrollable,
                       GtkBorder     *border)
{
  GxInhibitedScrollable *self = GX_INHIBITED_SCROLLABLE (scrollable);
...
{
  return g_object_new (GX_TYPE_INHIBITED_SCROLLABLE, NULL);
}
...
gx_inhibited_scrollable_set_scrollable (GxInhibitedScrollable *self,
                                        GtkScrollable         *child)
{
  g_return_if_fail (GX_IS_INHIBITED_SCROLLABLE (self));
...
{
  g_return_val_if_fail (GX_IS_INHIBITED_SCROLLABLE (self), NULL);
  return self->child;
}
