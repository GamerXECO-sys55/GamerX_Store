...
G_DEFINE_ENUM_TYPE (
    ...
    gx_dynamic_list_view_kind,
    G_DEFINE_ENUM_VALUE (GX_DYNAMIC_LIST_VIEW_KIND_LIST_BOX, "list-box"),
    G_DEFINE_ENUM_VALUE (GX_DYNAMIC_LIST_VIEW_KIND_FLOW_BOX, "flow-box"),
    G_DEFINE_ENUM_VALUE (GX_DYNAMIC_LIST_VIEW_KIND_CAROUSEL, "carousel"))

struct _GxDynamicListView
{
...
G_DEFINE_FINAL_TYPE (GxDynamicListView, gx_dynamic_list_view, ADW_TYPE_BIN);
...
static void
refresh (GxDynamicListView *self);

static void
list_item_factory_setup (GxDynamicListView        *self,
                         GtkListItem              *item,
                         GtkSignalListItemFactory *factory);

static void
list_item_factory_teardown (GxDynamicListView        *self,
                            GtkListItem              *item,
                            GtkSignalListItemFactory *factory);

static void
list_item_factory_bind (GxDynamicListView        *self,
                        GtkListItem              *item,
                        GtkSignalListItemFactory *factory);

static void
list_item_factory_unbind (GxDynamicListView        *self,
                          GtkListItem              *item,
                          GtkSignalListItemFactory *factory);

static GtkWidget *
create_child_widget (GObject           *object,
                     GxDynamicListView *self);
...
static void
gx_dynamic_list_view_dispose (GObject *object)
{
  GxDynamicListView *self = GX_DYNAMIC_LIST_VIEW (object);
  ...
  G_OBJECT_CLASS (gx_dynamic_list_view_parent_class)->dispose (object);
}

static void
gx_dynamic_list_view_get_property (GObject    *object,
                                   guint       prop_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MODEL:
      g_value_set_object (value, gx_dynamic_list_view_get_model (self));
      break;
    case PROP_SCROLL:
      g_value_set_boolean (value, gx_dynamic_list_view_get_scroll (self));
      break;
    case PROP_NOSCROLL_KIND:
      g_value_set_enum (value, gx_dynamic_list_view_get_noscroll_kind (self));
      break;
    case PROP_CHILD_TYPE:
      g_value_set_string (value, gx_dynamic_list_view_get_child_type (self));
      break;
    case PROP_CHILD_PROP:
      g_value_set_string (value, gx_dynamic_list_view_get_child_prop (self));
      break;
    case PROP_OBJECT_PROP:
      g_value_set_string (value, gx_dynamic_list_view_get_object_prop (self));
      break;
    case PROP_MAX_CHILDREN_PER_LINE:
      g_value_set_uint (value, gx_dynamic_list_view_get_max_children_per_line (self));
      break;
    default:
...
static void
gx_dynamic_list_view_set_property (GObject      *object,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MODEL:
      gx_dynamic_list_view_set_model (self, g_value_get_object (value));
      break;
    case PROP_SCROLL:
      gx_dynamic_list_view_set_scroll (self, g_value_get_boolean (value));
      break;
    case PROP_NOSCROLL_KIND:
      gx_dynamic_list_view_set_noscroll_kind (self, g_value_get_enum (value));
      break;
    case PROP_CHILD_TYPE:
      gx_dynamic_list_view_set_child_type (self, g_value_get_string (value));
      break;
    case PROP_CHILD_PROP:
      gx_dynamic_list_view_set_child_prop (self, g_value_get_string (value));
      break;
    case PROP_OBJECT_PROP:
      gx_dynamic_list_view_set_object_prop (self, g_value_get_string (value));
      break;
    case PROP_MAX_CHILDREN_PER_LINE:
      gx_dynamic_list_view_set_max_children_per_line (self, g_value_get_uint (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
gx_dynamic_list_view_class_init (GxDynamicListViewClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_dynamic_list_view_set_property;
  object_class->get_property = gx_dynamic_list_view_get_property;
  object_class->dispose      = gx_dynamic_list_view_dispose;
  ...
  props[PROP_NOSCROLL_KIND] =
      g_param_spec_enum (
          "noscroll-kind",
          NULL, NULL,
          GX_TYPE_DYNAMIC_LIST_VIEW_KIND, GX_DYNAMIC_LIST_VIEW_KIND_LIST_BOX,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
  ...
  signals[SIGNAL_BIND_WIDGET] =
      g_signal_new (
          ...
          0,
          NULL, NULL,
          gx_marshal_VOID__OBJECT_OBJECT,
          G_TYPE_NONE,
          1,
          GTK_TYPE_WIDGET);
  g_signal_set_va_marshaller (
      ...
      gx_marshal_VOID__OBJECT_OBJECTv);
...
static void
gx_dynamic_list_view_init (GxDynamicListView *self)
{
  self->child_type            = G_TYPE_INVALID;
  self->noscroll_kind         = GX_DYNAMIC_LIST_VIEW_KIND_LIST_BOX;
...
{
  return g_object_new (GX_TYPE_DYNAMIC_LIST_VIEW, NULL);
}

GListModel *
gx_dynamic_list_view_get_model (GxDynamicListView *self)
{
  g_return_val_if_fail (GX_IS_DYNAMIC_LIST_VIEW (self), NULL);
  return self->model;
}
...
{
  g_return_val_if_fail (GX_IS_DYNAMIC_LIST_VIEW (self), FALSE);
  return self->scroll;
}
...
{
  g_return_val_if_fail (GX_IS_DYNAMIC_LIST_VIEW (self), 4);
...
gx_dynamic_list_view_set_model (GxDynamicListView *self,
                                GListModel        *model)
{
  g_return_if_fail (GX_IS_DYNAMIC_LIST_VIEW (self));
...
{
  g_return_if_fail (GX_IS_DYNAMIC_LIST_VIEW (self));
  g_return_if_fail (noscroll_kind >= 0 && noscroll_kind < GX_DYNAMIC_LIST_VIEW_N_KINDS);
                                                ...
                                                guint              max_children)
{
  ...
  if (child != NULL &&
      self->noscroll_kind == GX_DYNAMIC_LIST_VIEW_KIND_FLOW_BOX &&
      !self->scroll)
    {
  ...
  if (self->scroll)
  ...
  else
    {
      switch (self->noscroll_kind)
        {
        case GX_DYNAMIC_LIST_VIEW_KIND_LIST_BOX:
          {
          ...
          break;
        case GX_DYNAMIC_LIST_VIEW_KIND_FLOW_BOX:
          {
            GtkWidget *widget = NULL;
          ...
          break;
        case GX_DYNAMIC_LIST_VIEW_KIND_CAROUSEL:
          {
            GtkWidget *widget = NULL;
          ...
          break;
        case GX_DYNAMIC_LIST_VIEW_N_KINDS:
        default:
                     ...
                     GxDynamicListView *self)
{
  ...
  if (self->noscroll_kind == GX_DYNAMIC_LIST_VIEW_KIND_FLOW_BOX)
    {
      GtkWidget *child = NULL;
