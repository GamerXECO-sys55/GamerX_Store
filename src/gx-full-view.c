...
#define G_LOG_DOMAIN "GAMERX_STORE::FULL-VIEW-WIDGET"
...
struct _GxFullView
{
  ...
  GxEntryGroup         *group;
  GxResult             *ui_entry;
  GxResult             *debounced_ui_entry;
  GxResult             *group_model;
...
G_DEFINE_FINAL_TYPE (GxFullView, gx_full_view, ADW_TYPE_BIN)
...
static void
debounce_timeout (GxFullView *self);

static DexFuture *
retrieve_star_string_fiber (GxFullView *self);

static void addon_transact_cb (GxFullView     *self,
                               GxEntry        *entry,
                               GxAddonsDialog *dialog);

static void
gx_full_view_dispose (GObject *object)
{
  GxFullView *self = GX_FULL_VIEW (object);
  ...
  G_OBJECT_CLASS (gx_full_view_parent_class)->dispose (object);
}

static void
gx_full_view_get_property (GObject    *object,
                           guint       prop_id,
                           GValue     *value,
                           GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_TRANSACTION_MANAGER:
      g_value_set_object (value, gx_full_view_get_transaction_manager (self));
      break;
    case PROP_ENTRY_GROUP:
      g_value_set_object (value, gx_full_view_get_entry_group (self));
      break;
    case PROP_UI_ENTRY:
...
static void
gx_full_view_set_property (GObject      *object,
                           guint         prop_id,
                           const GValue *value,
                           GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_STATE:
      ...
      break;
    case PROP_TRANSACTION_MANAGER:
      gx_full_view_set_transaction_manager (self, g_value_get_object (value));
      break;
    case PROP_ENTRY_GROUP:
      gx_full_view_set_entry_group (self, g_value_get_object (value));
      break;
    case PROP_UI_ENTRY:
    case PROP_DEBOUNCED_UI_ENTRY:
    default:
...
static void
open_url_cb (GxFullView   *self,
             AdwActionRow *row)
{
  GxEntry    *entry = NULL;
  const char *url   = NULL;

  entry = GX_ENTRY (gx_result_get_object (self->ui_entry));
  url   = gx_entry_get_url (entry);
          ...
          GtkButton  *button)
{
  ...
  share_dialog = gx_share_dialog_new (gx_result_get_object (self->ui_entry));
...
static void
dl_stats_cb (GxFullView *self,
             GtkButton  *button)
{
  AdwDialog *dialog   = NULL;
  GxEntry   *ui_entry = NULL;

  if (self->group == NULL)
    return;

  dialog = gx_stats_dialog_new (NULL);
  adw_dialog_set_content_width (dialog, 2000);
  adw_dialog_set_content_height (dialog, 1500);

  ui_entry = gx_result_get_object (self->ui_entry);
  g_object_bind_property (ui_entry, "download-stats", dialog, "model", G_BINDING_SYNC_CREATE);

  adw_dialog_present (dialog, GTK_WIDGET (self));
  gx_stats_dialog_animate_open (GX_STATS_DIALOG (dialog));
}

static void
run_cb (GxFullView *self,
        GtkButton  *button)
{
  GListModel *model   = NULL;
  guint       n_items = 0;

  if (self->group == NULL || !gx_result_get_resolved (self->group_model))
    return;

  model   = gx_result_get_object (self->group_model);
  ...
  for (guint i = 0; i < n_items; i++)
    {
      ...
      if (GX_IS_FLATPAK_ENTRY (entry) && gx_entry_is_installed (entry))
        {
...
static void
support_cb (GxFullView *self,
            GtkButton  *button)
{
  GxEntry *entry = NULL;

  entry = gx_result_get_object (self->ui_entry);
  if (entry != NULL)
    {
      const char *url = NULL;
  ...
  if (entry != NULL)
    {
      ...
      url = gx_entry_get_forge_url (entry);
      g_app_info_launch_default_for_uri (url, NULL, NULL);
    }
}

static void
install_addons_cb (GxFullView *self,
                   GtkButton  *button)
{
  GxEntry    *entry                   = NULL;
  GListModel *model                   = NULL;
  ...
  mapped_model = gx_application_map_factory_generate (
      gx_state_info_get_entry_factory (self->state),
      model);

  addons_dialog = gx_addons_dialog_new (entry, mapped_model);
...
static void
addon_transact_cb (GxFullView     *self,
                   GxEntry        *entry,
                   GxAddonsDialog *dialog)
{
...
static void
clear_releases_box (GxFullView *self)
{
...
static void
populate_releases_box (GxFullView *self)
{
  GxEntry *entry                         = NULL;
  ...
  entry = gx_result_get_object (self->debounced_ui_entry);
  if (entry == NULL)
    return;
...
static void
screenshots_bind_widget_cb (GxFullView            *self,
                            GxDecoratedScreenshot *screenshot,
...
static void
screenshots_unbind_widget_cb (GxFullView            *self,
                              GxDecoratedScreenshot *screenshot,
                              GdkPaintable          *paintable,
                              GxDynamicListView     *view)
{
...
static void
gx_full_view_class_init (GxFullViewClass *klass)
{
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose      = gx_full_view_dispose;
  object_class->get_property = gx_full_view_get_property;
  object_class->set_property = gx_full_view_set_property;
  ...
  g_type_ensure (GX_TYPE_DECORATED_SCREENSHOT);
  g_type_ensure (GX_TYPE_DYNAMIC_LIST_VIEW);
  g_type_ensure (GX_TYPE_ENTRY);
  g_type_ensure (GX_TYPE_ENTRY_GROUP);
  g_type_ensure (GX_TYPE_LAZY_ASYNC_TEXTURE_MODEL);
  ...
  gtk_widget_class_bind_template_child (widget_class, GxFullView, releases_box);
...
static void
gx_full_view_init (GxFullView *self)
{
...
{
  return g_object_new (GX_TYPE_FULL_VIEW, NULL);
}
...
gx_full_view_set_transaction_manager (GxFullView           *self,
                                      GxTransactionManager *transactions)
{
  g_return_if_fail (GX_IS_FULL_VIEW (self));
  g_return_if_fail (transactions == NULL ||
                    GX_IS_TRANSACTION_MANAGER (transactions));
...
{
  g_return_val_if_fail (GX_IS_FULL_VIEW (self), NULL);
  ...
  if (group != NULL)
    {
      ...
      self->ui_entry         = gx_entry_group_dup_ui_entry (group);
      self->debounce_timeout = g_timeout_add_once (
          300, (GSourceOnceFunc) debounce_timeout, self);

      future            = gx_entry_group_dup_all_into_model (group);
      self->group_model = gx_result_new (future);
...
debounce_timeout (GxFullView *self)
{
  ...
  if (gx_result_get_resolved (self->debounced_ui_entry))
  ...
  if (self->state != NULL &&
      g_settings_get_boolean (
          gx_state_info_get_settings (self->state),
...
retrieve_star_string_fiber (GxFullView *self)
{
  ...
  entry = dex_await_object (gx_result_dup_future (self->ui_entry), NULL);
  if (entry == NULL)
    goto done;

  forge_link = gx_entry_get_forge_url (entry);
  if (forge_link == NULL)
    goto done;
  ...
  node = dex_await_boxed (gx_https_query_json (star_url), &local_error);
  if (node == NULL)
    {
