...
G_DEFINE_FINAL_TYPE (GxSectionView, gx_section_view, ADW_TYPE_BIN)
...
static void
dark_changed (GxSectionView   *self,
              GParamSpec      *pspec,
              AdwStyleManager *mgr);

static void
refresh_dark_light_classes (GxSectionView   *self,
                            AdwStyleManager *mgr);

static void
gx_section_view_dispose (GObject *object)
{
  GxSectionView *self = GX_SECTION_VIEW (object);
  ...
  G_OBJECT_CLASS (gx_section_view_parent_class)->dispose (object);
}

static void
gx_section_view_get_property (GObject    *object,
                              guint       prop_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SECTION:
      g_value_set_object (value, gx_section_view_get_section (self));
      break;
    default:
...
static void
gx_section_view_set_property (GObject      *object,
                              guint         prop_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_SECTION:
      gx_section_view_set_section (self, g_value_get_object (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
bind_widget_cb (GxSectionView     *self,
                GxCuratedAppTile  *tile,
                GxEntryGroup      *group,
...
static void
unbind_widget_cb (GxSectionView     *self,
                  GxCuratedAppTile  *tile,
                  GxEntryGroup      *group,
                  GxDynamicListView *view)
{
...
static void
gx_section_view_class_init (GxSectionViewClass *klass)
{
  GObjectClass   *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose      = gx_section_view_dispose;
  object_class->get_property = gx_section_view_get_property;
  object_class->set_property = gx_section_view_set_property;
...
static void
gx_section_view_init (GxSectionView *self)
{
...
gx_section_view_set_section (GxSectionView    *self,
                             GxContentSection *section)
{
  g_return_if_fail (GX_IS_SECTION_VIEW (self));
  g_return_if_fail (section == NULL || GX_IS_CONTENT_SECTION (section));
...
{
  g_return_val_if_fail (GX_IS_SECTION_VIEW (self), NULL);
  return self->section;
}
              ...
              GtkButton    *button)
{
  ...
  self = gtk_widget_get_ancestor (GTK_WIDGET (button), GX_TYPE_SECTION_VIEW);
