...
struct _GxApplicationMapFactory
{
...
G_DEFINE_FINAL_TYPE (GxApplicationMapFactory, gx_application_map_factory, G_TYPE_OBJECT);

static void
gx_application_map_factory_dispose (GObject *object)
{
  GxApplicationMapFactory *self = GX_APPLICATION_MAP_FACTORY (object);
  ...
  G_OBJECT_CLASS (gx_application_map_factory_parent_class)->dispose (object);
}

static void
gx_application_map_factory_class_init (GxApplicationMapFactoryClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = gx_application_map_factory_dispose;
}

static void
gx_application_map_factory_init (GxApplicationMapFactory *self)
{
}
...
gx_application_map_factory_new (GtkMapListModelMapFunc func,
                                ...
                                GtkFilter             *filter)
{
  GxApplicationMapFactory *self = NULL;

  g_return_val_if_fail (func != NULL, NULL);
  g_return_val_if_fail (filter == NULL || GTK_IS_FILTER (filter), NULL);

  self                  = g_object_new (GX_TYPE_APPLICATION_MAP_FACTORY, NULL);
  self->func            = func;
                                     ...
                                     GListModel              *model)
{
  ...
  g_return_val_if_fail (GX_IS_APPLICATION_MAP_FACTORY (self), NULL);
