...
#define G_LOG_DOMAIN "GAMERX_STORE::CORE"

#include "config.h"

// #include <gamerx-store-ui.h>
#include <glib/gi18n.h>

#include "gx-application-map-factory.h"
#include "gx-application.h"
#include "gx-backend-notification.h"
#include "gx-content-provider.h"
#include "gx-download-worker.h"
...
#include "gx-preferences-dialog.h"
#include "gx-result.h"
...
struct _GxApplication
{
  ...
  GxEntryCacheManager        *cache;
  GxTransactionManager       *transactions;
  GxSearchEngine             *search_engine;
  GxGnomeShellSearchProvider *gs_search;
  ...
  GxContentProvider *content_provider;
...
G_DEFINE_FINAL_TYPE (GxApplication, gx_application, ADW_TYPE_APPLICATION)
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (self, g_object_unref);
    GX_RELEASE_DATA (file, g_object_unref))

static void
init_service_struct (GxApplication *self);

static DexFuture *
open_flatpakref_fiber (OpenFlatpakrefData *data);

static void
open_generic_id (GxApplication *self,
                 const char    *generic_id);

static void
transaction_success (GxApplication        *self,
                     GxTransaction        *transaction,
                     GHashTable           *errored,
                     GxTransactionManager *manager);

static DexFuture *
refresh_fiber (GxApplication *self);

static DexFuture *
watch_backend_notifs_fiber (GxApplication *self);

static void
refresh (GxApplication *self);

static gboolean
window_close_request (GxApplication *self,
                      GtkWidget     *window);

static GtkWindow *
new_window (GxApplication *self);

static void
open_appstream_take (GxApplication *self,
                     char          *appstream);

static void
open_flatpakref_take (GxApplication *self,
                      GFile         *file);

static void
command_line_open_location (GxApplication           *self,
                            GApplicationCommandLine *cmdline,
                            const char              *path);

static gint
cmp_group (GxEntryGroup *a,
           GxEntryGroup *b,
           gpointer      user_data);

static void
gx_application_dispose (GObject *object)
{
  GxApplication *self = GX_APPLICATION (object);
  ...
  G_OBJECT_CLASS (gx_application_parent_class)->dispose (object);
}

static void
gx_application_activate (GApplication *app)
{
}

static int
gx_application_command_line (GApplication            *app,
                             GApplicationCommandLine *cmdline)
{
  GxApplication *self               = GX_APPLICATION (app);
      ...
      if (!self->running || !window_autostart)
        {
          ...
          gx_state_info_set_blocklists (self->state, self->blocklists);
          gx_state_info_set_curated_configs (self->state, self->content_configs);
        }
      ...
      if (g_strcmp0 (command, "window") == 0)
        {
          ...
          if (search)
            gx_window_search (GX_WINDOW (window), search_text);
        }
      ...
      else if (g_strcmp0 (command, "status") == 0)
        {
          ...
          for (guint i = 0; i < n_transactions; i++)
            {
              g_autoptr (GxTransaction) transaction = NULL;
...
static gboolean
gx_application_local_command_line (GApplication *application,
                                   gchar      ***arguments,
                                   int          *exit_status)
{
  return FALSE;
...
static gboolean
gx_application_dbus_register (GApplication    *application,
                              GDBusConnection *connection,
                              const gchar     *object_path,
                              GError         **error)
{
  GxApplication *self = GX_APPLICATION (application);
  return gx_gnome_shell_search_provider_set_connection (self->gs_search, connection, error);
}

static void
gx_application_dbus_unregister (GApplication    *application,
                                GDBusConnection *connection,
                                const gchar     *object_path)
{
  GxApplication *self = GX_APPLICATION (application);
  gx_gnome_shell_search_provider_set_connection (self->gs_search, NULL, NULL);
}

static void
gx_application_class_init (GxApplicationClass *klass)
{
  GObjectClass      *object_class = G_OBJECT_CLASS (klass);
  GApplicationClass *app_class    = G_APPLICATION_CLASS (klass);

  object_class->dispose = gx_application_dispose;

  app_class->activate           = gx_application_activate;
  app_class->command_line       = gx_application_command_line;
  app_class->local_command_line = gx_application_local_command_line;
  app_class->dbus_register      = gx_application_dbus_register;
  app_class->dbus_unregister    = gx_application_dbus_unregister;

  g_type_ensure (GX_TYPE_RESULT);
}

static void
gx_application_bazaar_inspector_action (GSimpleAction *action,
                                        GVariant      *parameter,
                                        gpointer       user_data)
{
  GxApplication *self      = user_data;
  GxInspector   *inspector = NULL;

  g_assert (GX_IS_APPLICATION (self));

  inspector = gx_inspector_new ();
  gx_inspector_set_state (inspector, self->state);
...
static void
gx_application_flatseal_action (GSimpleAction *action,
                                GVariant      *parameter,
                                gpointer       user_data)
{
  GxApplication *self   = user_data;
...
static void
gx_application_donate_action (GSimpleAction *action,
                              GVariant      *parameter,
                              gpointer       user_data)
{
  GxApplication *self = user_data;
...
static void
gx_application_toggle_transactions_action (GSimpleAction *action,
                                           GVariant      *parameter,
                                           gpointer       user_data)
{
  GxApplication *self   = user_data;
  ...
  gx_window_toggle_transactions (GX_WINDOW (window));
}

static void
gx_application_search_action (GSimpleAction *action,
                              GVariant      *parameter,
                              gpointer       user_data)
{
  GxApplication *self         = user_data;
  ...
  gx_window_search (GX_WINDOW (window), initial_text);
}

static void
gx_application_about_action (GSimpleAction *action,
                             GVariant      *parameter,
                             gpointer       user_data)
{
  GxApplication   *self               = user_data;
  ...
  const char *developers[] = {
    C_ ("About Dialog Developer Credit", "Mangesh Choudhary"),
  ...
  g_object_set (
      dialog,
      "application-name", "GamerX Store",
      "application-icon", "io.github.gamerx.GamerXStore",
      "developer-name", _ ("GamerX"),
      "translator-credits", translators_string,
      "version", PACKAGE_VERSION,
      "developers", developers,
      "copyright", "© 2025 Mangesh Choudhary",
      "license-type", GTK_LICENSE_GPL_3_0,
      "website", "https://github.com/GamerXECO-sys55/GamerX_Store",
      "issue-url", "https://github.com/GamerXECO-sys55/GamerX_Store/issues",
      NULL);
...
static void
gx_application_preferences_action (GSimpleAction *action,
                                   GVariant      *parameter,
                                   gpointer       user_data)
{
  GxApplication *self        = user_data;
  ...
  window      = gtk_application_get_active_window (GTK_APPLICATION (self));
  preferences = gx_preferences_dialog_new (self->settings);

  adw_dialog_present (preferences, GTK_WIDGET (window));
}

static void
gx_application_refresh_action (GSimpleAction *action,
                               GVariant      *parameter,
                               gpointer       user_data)
{
  GxApplication *self = user_data;
...
static void
gx_application_quit_action (GSimpleAction *action,
                            GVariant      *parameter,
                            gpointer       user_data)
{
  GxApplication *self = user_data;
...
static const GActionEntry app_actions[] = {
  {                "quit",                gx_application_quit_action, NULL },
  {             "refresh",             gx_application_refresh_action, NULL },
  {         "preferences",         gx_application_preferences_action, NULL },
  {               "about",               gx_application_about_action, NULL },
  {              "search",              gx_application_search_action,  "s" },
  { "toggle-transactions", gx_application_toggle_transactions_action, NULL },
  {              "donate",              gx_application_donate_action, NULL },
  {            "flatseal",            gx_application_flatseal_action, NULL },
  {    "bazaar-inspector",    gx_application_bazaar_inspector_action, NULL },
};
                           ...
                           GxApplication   *self)
{
  ...
  if (gx_state_info_get_busy (self->state))
    return NULL;
                    ...
                    GxApplication   *self)
{
  ...
  future = gx_entry_cache_manager_get (self->cache, id);
  result = gx_result_new (future);
...
static void
gx_application_init (GxApplication *self)
{
  self->running = FALSE;
  g_weak_ref_init (&self->main_window, NULL);

  self->gs_search = gx_gnome_shell_search_provider_new ();
...
init_service_struct (GxApplication *self)
{
  ...
  if (config_bytes != NULL)
    {
      g_autoptr (GxYamlParser) parser      = NULL;
      g_autoptr (GHashTable) parse_results = NULL;

      parser = gx_yaml_parser_new_for_resource_schema (
          "/io/github/gamerx/GamerXStore/main-config-schema.xml");

      parse_results = gx_yaml_parser_process_bytes (
  ...
  (void) gx_download_worker_get_default ();
  ...
  self->groups         = g_list_store_new (GX_TYPE_ENTRY_GROUP);
  self->installed_apps = g_list_store_new (GX_TYPE_ENTRY_GROUP);
  self->ids_to_groups  = g_hash_table_new_full (
      g_str_hash, g_str_equal, g_free, g_object_unref);

  self->entry_factory = gx_application_map_factory_new (
  ...
  self->application_factory = gx_application_map_factory_new (
      (GtkMapListModelMapFunc) map_generic_ids_to_groups,
      self, NULL, NULL, GTK_FILTER (filter));

  self->search_engine = gx_search_engine_new ();
  gx_search_engine_set_model (self->search_engine, G_LIST_MODEL (self->groups));
  gx_gnome_shell_search_provider_set_engine (self->gs_search, self->search_engine);

  self->content_provider         = gx_content_provider_new ();
  self->content_configs_to_files = gtk_map_list_model_new (
      NULL, (GtkMapListModelMapFunc) map_strings_to_files, NULL, NULL);
  gx_content_provider_set_input_files (
      self->content_provider, G_LIST_MODEL (self->content_configs_to_files));
  gx_content_provider_set_factory (self->content_provider, self->application_factory);

  self->flathub = gx_flathub_state_new ();
  gx_flathub_state_set_map_factory (self->flathub, self->application_factory);

  self->transactions = gx_transaction_manager_new ();
  if (self->config != NULL)
    gx_transaction_manager_set_config (self->transactions, self->config);
  g_signal_connect_swapped (self->transactions, "success",
                            G_CALLBACK (transaction_success), self);

  self->state = gx_state_info_new ();
  gx_state_info_set_application_factory (self->state, self->application_factory);
  gx_state_info_set_blocklists (self->state, self->blocklists);
  gx_state_info_set_curated_provider (self->state, self->content_provider);
  gx_state_info_set_entry_factory (self->state, self->entry_factory);
  gx_state_info_set_flathub (self->state, self->flathub);
  gx_state_info_set_main_config (self->state, self->config);
  gx_state_info_set_search_engine (self->state, self->search_engine);
  gx_state_info_set_settings (self->state, self->settings);
  gx_state_info_set_transaction_manager (self->state, self->transactions);
}

static DexFuture *
open_flatpakref_fiber (OpenFlatpakrefData *data)
{
  GxApplication *self            = data->self;
  ...
  future = gx_backend_load_local_package (GX_BACKEND (self->flatpak), file, NULL);
      ...
      if (G_VALUE_HOLDS_OBJECT (value))
        {
          ...
          g_autoptr (GxEntry) equiv = NULL;

          entry     = g_value_get_object (value);
          unique_id = gx_entry_get_unique_id (entry);

          equiv = dex_await_object (
              gx_entry_cache_manager_get (self->cache, unique_id),
              NULL);

          if (equiv != NULL)
            {
              if (gx_entry_is_of_kinds (equiv, GX_ENTRY_KIND_APPLICATION))
                {
                  const char   *generic_id = NULL;
                  GxEntryGroup *group      = NULL;

                  generic_id = gx_entry_get_id (entry);
                  group      = g_hash_table_lookup (self->ids_to_groups, generic_id);

                  if (group != NULL)
                    gx_window_show_group (GX_WINDOW (window), group);
                  else
                    gx_window_show_entry (GX_WINDOW (window), equiv);
                }
              else
                gx_window_show_entry (GX_WINDOW (window), equiv);
            }
          else
            gx_window_show_entry (GX_WINDOW (window), entry);
        }
      else
        open_generic_id (self, g_value_get_string (value));
    }
  else
    gx_show_error_for_widget (GTK_WIDGET (window), local_error->message);
                 ...
                 const char    *generic_id)
{
  ...
  if (group != NULL)
  ...
  else
    {
      ...
      gx_show_error_for_widget (GTK_WIDGET (window), message);
    }
}
                     ...
                     GxTransactionManager *manager)
{
  ...
  installs = gx_transaction_get_installs (transaction);
  removals = gx_transaction_get_removals (transaction);
  ...
  for (guint i = 0; i < n_installs; i++)
    {
      ...
      gx_entry_set_installed (entry, TRUE);
      unique_id = gx_entry_get_unique_id (entry);
      ...
      if (gx_entry_is_of_kinds (entry, GX_ENTRY_KIND_APPLICATION))
        {
          ...
          group = g_hash_table_lookup (self->ids_to_groups, gx_entry_get_id (entry));
          if (group != NULL)
            {
      ...
      dex_future_disown (gx_entry_cache_manager_add (self->cache, entry));
    }
  ...
  for (guint i = 0; i < n_removals; i++)
    {
      ...
      gx_entry_set_installed (entry, FALSE);
      unique_id = gx_entry_get_unique_id (entry);
      ...
      if (gx_entry_is_of_kinds (entry, GX_ENTRY_KIND_APPLICATION))
        {
          ...
          group = g_hash_table_lookup (self->ids_to_groups, gx_entry_get_id (entry));
          if (group != NULL && !gx_entry_group_get_removable (group))
            {
              gboolean found    = FALSE;
              guint    position = 0;
...
static void
fiber_check_for_updates (GxApplication *self)
{
  g_autoptr (GError) local_error   = NULL;
  ...
  gx_state_info_set_checking_for_updates (self->state, TRUE);

  update_ids = dex_await_boxed (
      gx_backend_retrieve_update_ids (GX_BACKEND (self->flatpak), NULL),
      &local_error);
  window = gtk_application_get_active_window (GTK_APPLICATION (self));
  if (update_ids != NULL)
    {
      if (update_ids->len > 0)
        {
          ...
          for (guint i = 0; i < update_ids->len; i++)
            {
              ...
              g_ptr_array_add (futures, gx_entry_cache_manager_get (self->cache, unique_id));
            }
          ...
          gx_state_info_set_available_updates (self->state, G_LIST_MODEL (store));
          // if (window != NULL)
          //   gx_window_push_update_dialog (GX_WINDOW (window));
        }
    }
  else if (window != NULL)
    gx_show_error_for_widget (GTK_WIDGET (window), local_error->message);
  ...
  gx_state_info_set_checking_for_updates (self->state, FALSE);
}
...
refresh_fiber (GxApplication *self)
{
  ...
  if (self->flatpak == NULL)
    {
      gx_state_info_set_busy_step_label (self->state, _ ("Constructing Flatpak instance..."));
      g_debug ("Constructing flatpak instance for the first time...");
      self->flatpak = dex_await_object (gx_flatpak_instance_new (), &local_error);
      if (self->flatpak == NULL)
        return dex_future_new_for_error (g_steal_pointer (&local_error));
      gx_transaction_manager_set_backend (self->transactions, GX_BACKEND (self->flatpak));
      gx_state_info_set_backend (self->state, GX_BACKEND (self->flatpak));
    ...
    {
      gx_state_info_set_busy_step_label (self->state, _ ("Reusing last Flatpak instance..."));
      g_debug ("Reusing previous flatpak instance...");
    }

  has_flathub = dex_await_boolean (
      gx_flatpak_instance_has_flathub (self->flatpak, NULL),
      &local_error);
  if (local_error != NULL)
    return dex_future_new_for_error (g_steal_pointer (&local_error));
  ...
  if (has_flathub)
  ...
  else
    {
      ...
      if (response != NULL &&
        ...
        {
          result = dex_await (
              gx_flatpak_instance_ensure_has_flathub (self->flatpak, NULL),
              &local_error);
          if (!result)
            return dex_future_new_for_error (g_steal_pointer (&local_error));
  ...
  if (gx_state_info_get_flathub (self->state) != NULL)
    {
      g_debug ("Updating Flathub state...");
      gx_flathub_state_update_to_today (self->flathub);
    }

  busy_step_label = g_strdup_printf (_ ("Identifying installed entries..."));
  gx_state_info_set_busy_step_label (self->state, busy_step_label);
  g_clear_pointer (&busy_step_label, g_free);

  installed_set = dex_await_boxed (
      gx_backend_retrieve_install_ids (
          GX_BACKEND (self->flatpak), NULL),
      &local_error);
  ...
  sync_future = gx_backend_retrieve_remote_entries_with_blocklists (
      GX_BACKEND (self->flatpak),
      channel,
      self->blocklists,
      NULL, self, NULL);
  ...
  for (;;)
    {
      ...
      if (G_VALUE_HOLDS_OBJECT (value))
        {
          ...
          id        = gx_entry_get_id (entry);
          unique_id = gx_entry_get_unique_id (entry);
          user      = gx_flatpak_entry_is_user (GX_FLATPAK_ENTRY (entry));

          installed = g_hash_table_contains (installed_set, unique_id);
          gx_entry_set_installed (entry, installed);

          flatpak_id = gx_flatpak_entry_get_flatpak_id (GX_FLATPAK_ENTRY (entry));
          if (flatpak_id != NULL)
            {
                  ...
                  for (guint i = 0; i < addons->len; i++)
                    {
                      ...
                      gx_entry_append_addon (entry, addon_id);
                    }
                  g_hash_table_remove (
                      user
                          ? usr_name_to_addons
          ...
          if (gx_entry_is_of_kinds (entry, GX_ENTRY_KIND_ADDON))
            {
              const char *extension_of_what = NULL;

              extension_of_what = gx_flatpak_entry_get_addon_extension_of_ref (
                  GX_FLATPAK_ENTRY (entry));
              if (extension_of_what != NULL)
                {
                  GPtrArray *addons = NULL;

                  /* GxFlatpakInstance ensures addons come before applications */
                  addons = g_hash_table_lookup (
                      user
                          ? usr_name_to_addons
                          : sys_name_to_addons,
          ...
          cache_task = gx_entry_cache_manager_add (self->cache, entry);
      ...
      gx_state_info_set_busy_progress (self->state, (double) total / (double) out_of);
      busy_step_label     = g_strdup_printf (_ ("Receiving Entries"));
      busy_progress_label = g_strdup_printf (_ ("%'d of %'d"), total, out_of);
      gx_state_info_set_busy_step_label (self->state, busy_step_label);
      gx_state_info_set_busy_progress_label (self->state, busy_progress_label);
      g_clear_pointer (&busy_step_label, g_free);
    }
  g_clear_pointer (&self->last_installed_set, g_hash_table_unref);
  ...
  if (G_VALUE_HOLDS_STRING (sync_value))
    {
      ...
      window = gtk_application_get_active_window (GTK_APPLICATION (self));
      if (window != NULL)
        gx_show_error_for_widget (GTK_WIDGET (window), warning);
    }

  g_debug ("Finished synchronizing with remotes, notifying UI...");
  gx_state_info_set_online (self->state, TRUE);
  gx_state_info_set_all_entry_groups (self->state, G_LIST_MODEL (self->groups));
  gx_search_engine_set_model (self->search_engine, G_LIST_MODEL (self->groups));
  gx_state_info_set_busy (self->state, FALSE);

  gtk_filter_changed (GTK_FILTER (self->application_filter), GTK_FILTER_CHANGE_DIFFERENT);
  gx_state_info_set_all_installed_entry_groups (self->state, G_LIST_MODEL (self->installed_apps));
  ...
  gx_state_info_set_background_task_label (self->state, _ ("Checking for updates..."));
  fiber_check_for_updates (self);
  gx_state_info_set_background_task_label (self->state, NULL);
  ...
  for (;;)
    {
      ...
      channel = gx_backend_create_notification_channel (
          GX_BACKEND (self->flatpak));
      if (channel == NULL)
        break;
      ...
      for (;;)
        {
          g_autoptr (GError) local_error          = NULL;
          g_autoptr (GxBackendNotification) notif = NULL;
          g_autoptr (GHashTable) installed_set    = NULL;
          ...
          gx_state_info_set_background_task_label (self->state, _ ("Synchronizing..."));
                  ...
                  if (dex_future_is_resolved (future))
                    {
                      ...
                      group = g_hash_table_lookup (self->ids_to_groups, id);
                      if (group != NULL)
                        gx_entry_group_connect_living (group, entry);
                          ...
                          if (installed && !found)
                            g_list_store_insert_sorted (
                                ...
                                (GCompareDataFunc) cmp_group, NULL);
                                   ...
                                   gx_entry_group_get_removable (group) == 0)
                            g_list_store_remove (self->installed_apps, position);
                        }

                      g_ptr_array_add (
                          diff_writes,
                          gx_entry_cache_manager_add (self->cache, entry));
                    }
                }
                 ...
                 GxApplication *self)
{
  ...
  if (value != NULL)
  ...
  else
    {
      ...
      gx_state_info_set_online (self->state, FALSE);
      ...
      if (window != NULL)
        {
          ...
          error_string = g_strdup_printf (
              ...
              local_error->message);
          gx_show_error_for_widget (GTK_WIDGET (window), error_string);
        }
    }
...
refresh (GxApplication *self)
{
  ...
  gx_state_info_set_all_entry_groups (self->state, NULL);
  gx_state_info_set_all_installed_entry_groups (self->state, NULL);
  gx_state_info_set_flathub (self->state, NULL);
  gx_search_engine_set_model (self->search_engine, NULL);

  g_list_store_remove_all (self->groups);
  g_hash_table_remove_all (self->ids_to_groups);
  g_list_store_remove_all (self->installed_apps);

  gx_state_info_set_busy (self->state, TRUE);
  gx_state_info_set_busy_progress (self->state, 0.0);
  gx_state_info_set_available_updates (self->state, NULL);
  gx_state_info_set_online (self->state, FALSE);

  g_clear_object (&self->cache);
  self->cache = gx_entry_cache_manager_new ();
...
new_window (GxApplication *self)
{
  ...
  window = gx_window_new (self->state);
           ...
           gpointer      user_data)
{
  ...
  title_a = gx_entry_group_get_title (a);
  title_b = gx_entry_group_get_title (b);
