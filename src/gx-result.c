...
G_DEFINE_FINAL_TYPE (GxResult, gx_result, G_TYPE_OBJECT);
...
static void
gx_result_dispose (GObject *object)
{
  GxResult *self = GX_RESULT (object);
  ...
  G_OBJECT_CLASS (gx_result_parent_class)->dispose (object);
}

static void
gx_result_get_property (GObject    *object,
                        guint       prop_id,
                        GValue     *value,
                        GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_PENDING:
      g_value_set_boolean (value, gx_result_get_pending (self));
      break;
    case PROP_RESOLVED:
      g_value_set_boolean (value, gx_result_get_resolved (self));
      break;
    case PROP_REJECTED:
      g_value_set_boolean (value, gx_result_get_rejected (self));
      break;
    case PROP_OBJECT:
      g_value_set_object (value, gx_result_get_object (self));
      break;
    case PROP_MESSAGE:
      g_value_set_string (value, gx_result_get_message (self));
      break;
    default:
...
static void
gx_result_set_property (GObject      *object,
                        guint         prop_id,
                        const GValue *value,
                        GParamSpec   *pspec)
{
...
static void
gx_result_class_init (GxResultClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_result_set_property;
  object_class->get_property = gx_result_get_property;
  object_class->dispose      = gx_result_dispose;
...
gx_result_new (DexFuture *future)
{
  ...
  self = g_object_new (GX_TYPE_RESULT, NULL);
...
{
  g_return_val_if_fail (GX_IS_RESULT (self), FALSE);
...
{
  g_return_val_if_fail (GX_IS_RESULT (self), NULL);
  return self->object;
}
