...
#define G_LOG_DOMAIN  "GAMERX_STORE::CACHE"
#define GAMERX_MODULE "entry-cache"
...
GX_DEFINE_DATA (
    ...
    OngoingTask,
    {
      ...
      GxGuard *ongoing_gates[MAX_CONCURRENT_WRITES];
      ...
      GxGuard *alive_gate;
      GMutex   alive_mutex;
      GxGuard *reading_gate;
      GMutex   reading_mutex;
      GxGuard *writing_gate;
      GMutex   writing_mutex;
    },
    GX_RELEASE_DATA (scheduler, dex_unref);
    GX_RELEASE_DATA (init, dex_unref);
    GX_RELEASE_DATA (alive_hash, g_hash_table_unref);
    GX_RELEASE_DATA (writing_hash, g_hash_table_unref);
    GX_RELEASE_DATA (reading_hash, g_hash_table_unref);
    for (guint i = 0; i < G_N_ELEMENTS (self->ongoing_gates); i++)
        GX_RELEASE_DATA (ongoing_gates[i], gx_guard_destroy);
    ...
    GX_RELEASE_DATA (alive_gate, gx_guard_destroy);
    GX_RELEASE_DATA (reading_gate, gx_guard_destroy);
    GX_RELEASE_DATA (writing_gate, gx_guard_destroy);
    g_mutex_clear (&self->alive_mutex);
    g_mutex_clear (&self->reading_mutex);
    g_mutex_clear (&self->writing_mutex););

struct _GxEntryCacheManager
{
...
G_DEFINE_FINAL_TYPE (GxEntryCacheManager, gx_entry_cache_manager, G_TYPE_OBJECT);
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (gate, gx_guard_destroy);
    g_mutex_clear (&self->mutex);
    g_weak_ref_clear (&self->wr);
    GX_RELEASE_DATA (cached, g_timer_destroy));
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (task_data, ongoing_task_data_unref);
    GX_RELEASE_DATA (unique_id_checksum, g_free);
...
static void
gx_entry_cache_manager_dispose (GObject *object)
{
  GxEntryCacheManager *self = GX_ENTRY_CACHE_MANAGER (object);

  dex_clear (&self->scheduler);
  dex_clear (&self->watch_task);
  g_clear_pointer (&self->task_data, ongoing_task_data_unref);

  G_OBJECT_CLASS (gx_entry_cache_manager_parent_class)->dispose (object);
}

static void
gx_entry_cache_manager_get_property (GObject    *object,
                                     guint       prop_id,
                                     GValue     *value,
                                     GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_MAX_MEMORY_USAGE:
      g_value_set_uint64 (value, gx_entry_cache_manager_get_max_memory_usage (self));
      break;
    default:
...
static void
gx_entry_cache_manager_set_property (GObject      *object,
                                     guint         prop_id,
                                     const GValue *value,
                                     GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_MAX_MEMORY_USAGE:
      gx_entry_cache_manager_set_max_memory_usage (self, g_value_get_uint64 (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
gx_entry_cache_manager_class_init (GxEntryCacheManagerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_entry_cache_manager_set_property;
  object_class->get_property = gx_entry_cache_manager_get_property;
  object_class->dispose      = gx_entry_cache_manager_dispose;
...
static void
gx_entry_cache_manager_init (GxEntryCacheManager *self)
{
...
{
  return g_object_new (GX_TYPE_ENTRY_CACHE_MANAGER, NULL);
}

guint64
gx_entry_cache_manager_get_max_memory_usage (GxEntryCacheManager *self)
{
  g_return_val_if_fail (GX_IS_ENTRY_CACHE_MANAGER (self), 0);
...
gx_entry_cache_manager_set_max_memory_usage (GxEntryCacheManager *self,
                                             guint64              max_memory_usage)
{
  g_return_if_fail (GX_IS_ENTRY_CACHE_MANAGER (self));
                            ...
                            GxEntry             *entry)
{
  ...
  dex_return_error_if_fail (GX_IS_ENTRY_CACHE_MANAGER (self));
  dex_return_error_if_fail (GX_IS_ENTRY (entry));
  dex_return_error_if_fail (!gx_entry_is_holding (entry));

  data                     = write_task_data_new ();
  data->task_data          = ongoing_task_data_ref (self->task_data);
  data->unique_id_checksum = g_strdup (gx_entry_get_unique_id_checksum (entry));
...
write_task_fiber (WriteTaskData *data)
{
  ...
  GxEntry         *entry               = data->entry;
  g_autoptr (GError) local_error       = NULL;
  g_autoptr (GxGuard) slot_guard       = NULL;
  g_autoptr (GxGuard) other_guard      = NULL;
  ...
  if (!GX_IS_FLATPAK_ENTRY (entry))
    return dex_future_new_reject (
        GX_ENTRY_CACHE_ERROR,
        GX_ENTRY_CACHE_ERROR_CACHE_FAILED,
  ...
  GX_BEGIN_GUARD_WITH_CONTEXT (&slot_guard,
  ...
  GX_BEGIN_GUARD_WITH_CONTEXT (&other_guard,
...
read_task_fiber (ReadTaskData *data)
{
  ...
  GX_BEGIN_GUARD_WITH_CONTEXT (&guard,
                               &task_data->writing_mutex,
                               &task_data->writing_gate);
  {
    writing_future = g_hash_table_lookup (task_data->writing_hash, unique_id_checksum);
                               ...
                               &task_data->alive_gate);
  {
    ...
    if (living != NULL)
      {
        g_autoptr (GxEntry) living_entry = NULL;

        living_entry_data_ref (living);
        gx_clear_guard (&guard);

        GX_BEGIN_GUARD_WITH_CONTEXT (&guard, &living->mutex, &living->gate);
  ...
  entry  = g_object_new (GX_TYPE_FLATPAK_ENTRY, NULL);
  result = gx_serializable_deserialize (GX_SERIALIZABLE (entry), variant, &local_error);
  if (!result)
    {
      ret_error = g_error_new (
          GX_ENTRY_CACHE_ERROR,
  ...
  for (;;)
    {
      ...
      GX_BEGIN_GUARD_WITH_CONTEXT (&guard0, &task_data->alive_mutex, &task_data->alive_gate);
      GX_BEGIN_GUARD_WITH_CONTEXT (&guard0, &task_data->reading_mutex, &task_data->reading_gate);
      GX_BEGIN_GUARD_WITH_CONTEXT (&guard0, &task_data->writing_mutex, &task_data->writing_gate);
      ...
      for (;;)
        {
          ...
          GX_BEGIN_GUARD_WITH_CONTEXT (&guard1, &living->mutex, &living->gate);
