...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (g, dex_unref);
    g_mutex_clear (&self->m);)

struct _GxDownloadWorker
{
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (subprocess, g_object_unref);
    GX_RELEASE_DATA (waiting, g_hash_table_unref);
    GX_RELEASE_DATA (mutex, mutex_box_data_unref));
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (promise, dex_unref);
    GX_RELEASE_DATA (src, g_object_unref);
    GX_RELEASE_DATA (dest, g_object_unref);
    GX_RELEASE_DATA (subprocess, g_object_unref);
    GX_RELEASE_DATA (waiting, g_hash_table_unref);
    GX_RELEASE_DATA (mutex, mutex_box_data_unref));
static DexFuture *
...
static void
gx_download_worker_dispose (GObject *object)
{
  GxDownloadWorker *self         = GX_DOWNLOAD_WORKER (object);
  ...
  G_OBJECT_CLASS (gx_download_worker_parent_class)->dispose (object);
}

static void
gx_download_worker_get_property (GObject    *object,
                                 guint       prop_id,
                                 GValue     *value,
                                 GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_NAME:
      g_value_set_string (value, gx_download_worker_get_name (self));
      break;
    default:
...
static void
gx_download_worker_set_property (GObject      *object,
                                 guint         prop_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_NAME:
      gx_download_worker_set_name (self, g_value_get_string (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
    }
...
static void
gx_download_worker_class_init (GxDownloadWorkerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_download_worker_set_property;
  object_class->get_property = gx_download_worker_get_property;
  object_class->dispose      = gx_download_worker_dispose;
...
static void
gx_download_worker_init (GxDownloadWorker *self)
{
...
static gboolean
gx_download_worker_initable_init (GInitable    *initable,
                                  GCancellable *cancellable,
                                  GError      **error)
{
  GxDownloadWorker *self             = GX_DOWNLOAD_WORKER (initable);
...
initable_iface_init (GInitableIface *iface)
{
  iface->init = gx_download_worker_initable_init;
}
...
{
  g_return_val_if_fail (GX_IS_DOWNLOAD_WORKER (self), NULL);
  return self->name;
}
...
gx_download_worker_set_name (GxDownloadWorker *self,
                             const char       *name)
{
  g_return_if_fail (GX_IS_DOWNLOAD_WORKER (self));
                           ...
                           GFile            *dest)
{
  ...
  dex_return_error_if_fail (GX_IS_DOWNLOAD_WORKER (self));
...
#define N_WORKERS 5
      ...
      for (guint i = 0; i < N_WORKERS; i++)
        {
          ...
          g_autoptr (GxDownloadWorker) worker = NULL;

          worker = gx_download_worker_new ("default", &local_error);
          if (worker == NULL)
  ...
  for (guint i = 0; i < workers->len; i++)
    {
      GxDownloadWorker **loc = NULL;

      loc = (GxDownloadWorker **) &g_ptr_array_index (workers, i);
  ...
  for (;;)
    {
      ...
      GX_BEGIN_GUARD_WITH_CONTEXT (&guard, &mutex->m, &mutex->g);
      {
