...
#define G_LOG_DOMAIN  "GAMERX_STORE::FLATPAK"
#define GAMERX_MODULE "flatpak"
...
#include "gx-env.h"
#include "gx-flatpak-private.h"
#include "gx-global-state.h"
#include "gx-io.h"
#include "gx-util.h"

/* clang-format off */
G_DEFINE_QUARK (gx-flatpak-error-quark, gx_flatpak_error);
/* clang-format on */

struct _GxFlatpakInstance
{
...
static void
backend_iface_init (GxBackendInterface *iface);

G_DEFINE_FINAL_TYPE_WITH_CODE (
    GxFlatpakInstance,
    gx_flatpak_instance,
    G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE (GX_TYPE_BACKEND, backend_iface_init));

GX_DEFINE_DATA (
    init,
    Init,
    { GxFlatpakInstance *instance; },
    GX_RELEASE_DATA (instance, g_object_unref))
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (cancellable, g_object_unref));
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (cancellable, g_object_unref);
    GX_RELEASE_DATA (channel, dex_unref);
    GX_RELEASE_DATA (blocked_names, g_ptr_array_unref);
    GX_RELEASE_DATA (user_data, self->destroy_user_data))
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (parent, gather_refs_data_unref);
    GX_RELEASE_DATA (installation, g_object_unref);
    GX_RELEASE_DATA (remote, g_object_unref);
    GX_RELEASE_DATA (blocked_names_hash, g_hash_table_unref));
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (cancellable, g_object_unref);
    GX_RELEASE_DATA (installs, g_ptr_array_unref);
    GX_RELEASE_DATA (updates, g_ptr_array_unref);
    GX_RELEASE_DATA (removals, g_ptr_array_unref);
    GX_RELEASE_DATA (channel, dex_unref);
    GX_RELEASE_DATA (send_futures, g_ptr_array_unref);
    GX_RELEASE_DATA (ref_to_entry_hash, g_hash_table_unref);
    GX_RELEASE_DATA (op_to_progress_hash, g_hash_table_unref));
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (parent, transaction_data_unref);
    GX_RELEASE_DATA (transaction, g_object_unref));
static DexFuture *
...
GX_DEFINE_DATA (
    ...
    {
      TransactionData               *parent;
      GxFlatpakEntry                *entry;
      GxBackendTransactionOpPayload *op;
    },
    GX_RELEASE_DATA (parent, transaction_data_unref);
    GX_RELEASE_DATA (entry, g_object_unref);
    GX_RELEASE_DATA (op, g_object_unref));
static void
transaction_progress_changed (FlatpakTransactionProgress *object,
                              TransactionOperationData   *data);

static void
installation_event (GxFlatpakInstance *self,
                    GFile             *file,
                    GFile             *other_file,
...
static void
gx_flatpak_instance_dispose (GObject *object)
{
  GxFlatpakInstance *self = GX_FLATPAK_INSTANCE (object);
  ...
  G_OBJECT_CLASS (gx_flatpak_instance_parent_class)->dispose (object);
}

static void
gx_flatpak_instance_class_init (GxFlatpakInstanceClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = gx_flatpak_instance_dispose;
}

static void
gx_flatpak_instance_init (GxFlatpakInstance *self)
{
...
static DexChannel *
gx_flatpak_instance_create_notification_channel (GxBackend *backend)
{
  GxFlatpakInstance *self        = GX_FLATPAK_INSTANCE (backend);
...
static DexFuture *
gx_flatpak_instance_load_local_package (GxBackend    *backend,
                                        GFile        *file,
                                        GCancellable *cancellable)
{
  GxFlatpakInstance *self           = GX_FLATPAK_INSTANCE (backend);
...
static DexFuture *
gx_flatpak_instance_retrieve_remote_refs (GxBackend     *backend,
                                          DexChannel    *channel,
                                          GPtrArray     *blocked_names,
                                          GCancellable  *cancellable,
                                          gpointer       user_data,
...
static DexFuture *
gx_flatpak_instance_retrieve_install_ids (GxBackend    *backend,
                                          GCancellable *cancellable)
{
  GxFlatpakInstance *self         = GX_FLATPAK_INSTANCE (backend);
  g_autoptr (GatherRefsData) data = NULL;
...
static DexFuture *
gx_flatpak_instance_retrieve_update_ids (GxBackend    *backend,
                                         GCancellable *cancellable)
{
  GxFlatpakInstance *self         = GX_FLATPAK_INSTANCE (backend);
  g_autoptr (GatherRefsData) data = NULL;
...
static DexFuture *
gx_flatpak_instance_schedule_transaction (GxBackend    *backend,
                                          GxEntry     **installs,
                                          guint         n_installs,
                                          GxEntry     **updates,
                                          guint         n_updates,
...
{
  GxFlatpakInstance *self          = GX_FLATPAK_INSTANCE (backend);
  GxFlatpakEntry   **installs_dup  = NULL;
  GxFlatpakEntry   **updates_dup   = NULL;
  GxFlatpakEntry   **removals_dup  = NULL;
  g_autoptr (TransactionData) data = NULL;

  if (n_installs > 0)
    {
      for (guint i = 0; i < n_installs; i++)
        g_return_val_if_fail (GX_IS_FLATPAK_ENTRY (installs[i]), NULL);
    }
  if (n_updates > 0)
    {
      for (guint i = 0; i < n_updates; i++)
        g_return_val_if_fail (GX_IS_FLATPAK_ENTRY (updates[i]), NULL);
    }
  if (n_removals > 0)
    {
      for (guint i = 0; i < n_removals; i++)
        g_return_val_if_fail (GX_IS_FLATPAK_ENTRY (removals[i]), NULL);
    }
  ...
  if (n_installs > 0)
    {
      ...
      for (guint i = 0; i < n_installs; i++)
        installs_dup[i] = g_object_ref (GX_FLATPAK_ENTRY (installs[i]));
    }
  if (n_updates > 0)
    {
      updates_dup = g_malloc0_n (n_updates, sizeof (*updates_dup));
      for (guint i = 0; i < n_updates; i++)
        updates_dup[i] = g_object_ref (GX_FLATPAK_ENTRY (updates[i]));
    }
  if (n_removals > 0)
    {
      removals_dup = g_malloc0_n (n_removals, sizeof (*removals_dup));
      for (guint i = 0; i < n_removals; i++)
        removals_dup[i] = g_object_ref (GX_FLATPAK_ENTRY (removals[i]));
    }
...
backend_iface_init (GxBackendInterface *iface)
{
  iface->create_notification_channel = gx_flatpak_instance_create_notification_channel;
  iface->load_local_package          = gx_flatpak_instance_load_local_package;
  iface->retrieve_remote_entries     = gx_flatpak_instance_retrieve_remote_refs;
  iface->retrieve_install_ids        = gx_flatpak_instance_retrieve_install_ids;
  iface->retrieve_update_ids         = gx_flatpak_instance_retrieve_update_ids;
  iface->schedule_transaction        = gx_flatpak_instance_schedule_transaction;
}

FlatpakInstallation *
gx_flatpak_instance_get_system_installation (GxFlatpakInstance *self)
{
  g_return_val_if_fail (GX_IS_FLATPAK_INSTANCE (self), NULL);
  return self->system;
}
...
gx_flatpak_instance_new (void)
{
  ...
  data           = init_data_new ();
  data->instance = g_object_new (GX_TYPE_FLATPAK_INSTANCE, NULL);
                                 ...
                                 GCancellable      *cancellable)
{
  ...
  dex_return_error_if_fail (GX_IS_FLATPAK_INSTANCE (self));
...
init_fiber (InitData *data)
{
  GxFlatpakInstance *instance    = data->instance;
  g_autoptr (GError) local_error = NULL;
  g_autofree char *main_cache    = NULL;

  gx_discard_module_dir ();
  ...
  if (instance->system == NULL && instance->user == NULL)
    return dex_future_new_reject (
        GX_FLATPAK_ERROR,
        GX_FLATPAK_ERROR_CANNOT_INITIALIZE,
...
#define REPO_URL "https://dl.flathub.org/repo/flathub.flatpakrepo"
  ...
  if (remote != NULL)
  ...
  else
    {
      ...
      result  = dex_await (
          gx_send_with_global_http_session_then_splice_into (message, output),
          &local_error);
      if (!result)
        return dex_future_new_reject (
            GX_FLATPAK_ERROR,
...
load_local_ref_fiber (LoadLocalRefData *data)
{
  ...
  g_autoptr (GxFlatpakEntry) entry  = NULL;
  ...
  entry = gx_flatpak_entry_new_for_ref (
      instance,
      FALSE,
      NULL,
      FLATPAK_REF (bref),
...
retrieve_refs_for_remote_fiber (RetrieveRefsForRemoteData *data)
{
  ...
  GxFlatpakInstance   *instance           = data->parent->instance;
  ...
  for (guint i = 0; i < n_system_refs + n_user_refs; i++)
    {
      ...
      g_hash_table_add (ids, gx_flatpak_ref_format_unique (FLATPAK_REF (iref), user));
    }
  ...
  for (guint i = 0; i < system_refs->len + user_refs->len; i++)
    {
      ...
      g_ptr_array_index (ids, i) =
          gx_flatpak_ref_format_unique (FLATPAK_REF (iref), user);
    }
      ...
      for (guint i = 0; i < installations->len; i++)
        {
          ...
          ref     = gx_flatpak_entry_get_ref (entry);
          is_user = gx_flatpak_entry_is_user (GX_FLATPAK_ENTRY (entry));
          ...
          result = flatpak_transaction_add_install (
              transaction,
              gx_entry_get_remote_repo_name (GX_ENTRY (entry)),
              ref_fmt,
              NULL,
              &local_error);
          if (!result)
...
{
  GxFlatpakEntry *entry                               = NULL;
  g_autoptr (GxBackendTransactionOpPayload) payload   = NULL;
  ...
  payload = gx_backend_transaction_op_payload_new ();
  gx_backend_transaction_op_payload_set_entry (
      payload, GX_ENTRY (entry));
  gx_backend_transaction_op_payload_set_name (
      payload, flatpak_transaction_operation_get_ref (operation));
  gx_backend_transaction_op_payload_set_download_size (
      payload, flatpak_transaction_operation_get_download_size (operation));
  gx_backend_transaction_op_payload_set_installed_size (
...
{
  TransactionData *parent                                   = data->parent;
  g_autoptr (GxBackendTransactionOpProgressPayload) payload = NULL;
  int            int_progress                               = 0;
  ...
  payload = gx_backend_transaction_op_progress_payload_new ();
  gx_backend_transaction_op_progress_payload_set_op (
      payload, data->op);
  gx_backend_transaction_op_progress_payload_set_status (
      payload, flatpak_transaction_progress_get_status (progress));
  gx_backend_transaction_op_progress_payload_set_is_estimating (
      payload, flatpak_transaction_progress_get_is_estimating (progress));
  gx_backend_transaction_op_progress_payload_set_progress (
      payload, double_progress);
  gx_backend_transaction_op_progress_payload_set_total_progress (
      payload, total_progress);
  gx_backend_transaction_op_progress_payload_set_bytes_transferred (
      payload, flatpak_transaction_progress_get_bytes_transferred (progress));
  gx_backend_transaction_op_progress_payload_set_start_time (
  ...
  if (self->notif_channels->len > 0)
    {
      ...
      notif = gx_backend_notification_new ();
      gx_backend_notification_set_kind (notif, GX_BACKEND_NOTIFICATION_KIND_ANY);
