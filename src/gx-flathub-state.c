...
#define G_LOG_DOMAIN "GAMERX_STORE::FLATHUB"
...
G_DEFINE_FINAL_TYPE (GxFlathubState, gx_flathub_state, G_TYPE_OBJECT);

static GListModel *gx_flathub_state_dup_apps_of_the_day_week (GxFlathubState *self);
...
static DexFuture *
initialize_fiber (GxFlathubState *self);
static DexFuture *
initialize_finally (DexFuture      *future,
                    GxFlathubState *self);

static void
gx_flathub_state_dispose (GObject *object)
{
  GxFlathubState *self = GX_FLATHUB_STATE (object);
  ...
  G_OBJECT_CLASS (gx_flathub_state_parent_class)->dispose (object);
}

static void
gx_flathub_state_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FOR_DAY:
      g_value_set_string (value, gx_flathub_state_get_for_day (self));
      break;
    case PROP_MAP_FACTORY:
      g_value_set_object (value, gx_flathub_state_get_map_factory (self));
      break;
    case PROP_APP_OF_THE_DAY:
      g_value_set_string (value, gx_flathub_state_get_app_of_the_day (self));
      break;
    case PROP_APP_OF_THE_DAY_GROUP:
      g_value_take_object (value, gx_flathub_state_dup_app_of_the_day_group (self));
      break;
    case PROP_APPS_OF_THE_WEEK:
      g_value_take_object (value, gx_flathub_state_dup_apps_of_the_week (self));
      break;
    case PROP_APPS_OF_THE_DAY_WEEK:
      g_value_take_object (value, gx_flathub_state_dup_apps_of_the_day_week (self));
      break;
    case PROP_CATEGORIES:
      g_value_set_object (value, gx_flathub_state_get_categories (self));
      break;
    case PROP_RECENTLY_UPDATED:
      g_value_take_object (value, gx_flathub_state_dup_recently_updated (self));
      break;
    case PROP_RECENTLY_ADDED:
      g_value_take_object (value, gx_flathub_state_dup_recently_added (self));
      break;
    case PROP_POPULAR:
      g_value_take_object (value, gx_flathub_state_dup_popular (self));
      break;
    case PROP_TRENDING:
      g_value_take_object (value, gx_flathub_state_dup_trending (self));
      break;
    default:
...
static void
gx_flathub_state_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FOR_DAY:
      gx_flathub_state_set_for_day (self, g_value_get_string (value));
      break;
    case PROP_MAP_FACTORY:
      gx_flathub_state_set_map_factory (self, g_value_get_object (value));
      break;
    case PROP_APP_OF_THE_DAY:
    case PROP_APP_OF_THE_DAY_GROUP:
    case PROP_APPS_OF_THE_WEEK:
...
static void
gx_flathub_state_class_init (GxFlathubStateClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_flathub_state_set_property;
  object_class->get_property = gx_flathub_state_get_property;
  object_class->dispose      = gx_flathub_state_dispose;
...
static void
gx_flathub_state_init (GxFlathubState *self)
{
}
...
{
  return g_object_new (GX_TYPE_FLATHUB_STATE, NULL);
}

const char *
gx_flathub_state_get_for_day (GxFlathubState *self)
{
  g_return_val_if_fail (GX_IS_FLATHUB_STATE (self), NULL);
  return self->for_day;
}
...
gx_flathub_state_dup_app_of_the_day_group (GxFlathubState *self)
{
  ...
  return gx_application_map_factory_convert_one (self->map_factory, string);
}
...
GListModel *
gx_flathub_state_dup_apps_of_the_day_week (GxFlathubState *self)
{
  ...
  if (self->map_factory != NULL)
    return gx_application_map_factory_generate (self->map_factory, G_LIST_MODEL (combined_list));
  else
...
gx_flathub_state_set_for_day (GxFlathubState *self,
                              const char     *for_day)
{
  g_return_if_fail (GX_IS_FLATHUB_STATE (self));
  ...
  if (for_day != NULL)
    {
      ...
      self->categories       = g_list_store_new (GX_TYPE_FLATHUB_CATEGORY);
...
gx_flathub_state_update_to_today (GxFlathubState *self)
{
  ...
  gx_flathub_state_set_for_day (self, for_day);
}
...
#define ADD_REQUEST(key, ...)                  \
  ...
  {                                            \
    ...
    _future  = gx_query_flathub_v2_json_take ( \
      ...
      while (g_hash_table_size (futures) > 0)
        {
          ...
          g_autoptr (DexFuture) future           = NULL;
          g_autoptr (JsonNode) node              = NULL;
          g_autoptr (GxFlathubCategory) category = NULL;
          ...
          category = gx_flathub_category_new ();
          store    = gtk_string_list_new (NULL);
          gx_flathub_category_set_name (category, name);
          gx_flathub_category_set_applications (category, G_LIST_MODEL (store));
