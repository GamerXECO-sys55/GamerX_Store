...
G_DEFINE_INTERFACE (GxBackend, gx_backend, G_TYPE_OBJECT)
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (blocklists, g_ptr_array_unref);
...
static DexChannel *
gx_backend_real_create_notification_channel (GxBackend *self)
{
  return NULL;
}

static DexFuture *
gx_backend_real_load_local_package (GxBackend    *self,
                                    GFile        *file,
                                    GCancellable *cancellable)
{
...
static DexFuture *
gx_backend_real_retrieve_remote_entries (GxBackend     *self,
                                         DexChannel    *channel,
...
static DexFuture *
gx_backend_real_retrieve_install_ids (GxBackend    *self,
                                      GCancellable *cancellable)
{
  return dex_future_new_reject (G_IO_ERROR, G_IO_ERROR_UNKNOWN, "Unimplemented");
}
...
static DexFuture *
gx_backend_real_retrieve_update_ids (GxBackend    *self,
                                     GCancellable *cancellable)
{
  return dex_future_new_reject (G_IO_ERROR, G_IO_ERROR_UNKNOWN, "Unimplemented");
}
...
static DexFuture *
gx_backend_real_schedule_transaction (GxBackend    *self,
                                      GxEntry     **installs,
                                      guint         n_installs,
                                      GxEntry     **updates,
                                      guint         n_updates,
...
static void
gx_backend_default_init (GxBackendInterface *iface)
{
  iface->create_notification_channel = gx_backend_real_create_notification_channel;
  iface->load_local_package          = gx_backend_real_load_local_package;
  iface->retrieve_remote_entries     = gx_backend_real_retrieve_remote_entries;
  iface->retrieve_install_ids        = gx_backend_real_retrieve_install_ids;
  iface->retrieve_update_ids         = gx_backend_real_retrieve_update_ids;
  iface->schedule_transaction        = gx_backend_real_schedule_transaction;
}

DexChannel *
gx_backend_create_notification_channel (GxBackend *self)
{
  g_return_val_if_fail (GX_IS_BACKEND (self), NULL);

  return GX_BACKEND_GET_IFACE (self)->create_notification_channel (self);
}
...
gx_backend_load_local_package (GxBackend    *self,
                               ...
                               GCancellable *cancellable)
{
  dex_return_error_if_fail (GX_IS_BACKEND (self));
  dex_return_error_if_fail (G_IS_FILE (file));
  dex_return_error_if_fail (cancellable == NULL || G_IS_CANCELLABLE (self));

  return GX_BACKEND_GET_IFACE (self)->load_local_package (self, file, cancellable);
}
                                    ...
                                    GDestroyNotify destroy_user_data)
{
  ...
  return GX_BACKEND_GET_IFACE (self)->retrieve_remote_entries (
      self,
      channel,
      blocked_names,
      cancellable,
...
{
  dex_return_error_if_fail (GX_IS_BACKEND (self));
  return GX_BACKEND_GET_IFACE (self)->retrieve_install_ids (self, cancellable);
}
...
{
  dex_return_error_if_fail (GX_IS_BACKEND (self));
  return GX_BACKEND_GET_IFACE (self)->retrieve_update_ids (self, cancellable);
}
  ...
  if (installs != NULL)
    {
      for (guint i = 0; i < n_installs; i++)
        dex_return_error_if_fail (GX_IS_ENTRY (installs[i]));
    }
  if (updates != NULL)
    {
      for (guint i = 0; i < n_updates; i++)
        dex_return_error_if_fail (GX_IS_ENTRY (updates[i]));
    }
  if (removals != NULL)
    {
      for (guint i = 0; i < n_removals; i++)
        dex_return_error_if_fail (GX_IS_ENTRY (removals[i]));
    }

  return GX_BACKEND_GET_IFACE (self)->schedule_transaction (
      self,
      installs,
      n_installs,
      updates,
                                            ...
                                            GCancellable *cancellable)
{
  ...
  for (guint i = 0; i < n_items; i++)
    {
      ...
      updates     = gx_transaction_get_updates (transaction);
  ...
  return gx_backend_schedule_transaction (
      self,
...
retrieve_with_blocklists_fiber (RetrieveWithBlocklistsData *data)
{
  ...
  return GX_BACKEND_GET_IFACE (data->backend)
