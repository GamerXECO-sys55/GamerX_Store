...
#define G_LOG_DOMAIN "GAMERX_STORE::GLOBAL-NET"
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (message, g_object_unref);
    GX_RELEASE_DATA (splice_into, g_object_unref));
static DexFuture *
...
DexFuture *
gx_send_with_global_http_session (SoupMessage *message)
{
...
DexFuture *
gx_send_with_global_http_session_then_splice_into (SoupMessage   *message,
...
DexFuture *
gx_https_query_json (const char *uri)
{
  ...
  soup_message_headers_append (headers, "User-Agent", "GamerX Store");
...
DexFuture *
gx_query_flathub_v2_json (const char *request)
{
  ...
  return gx_https_query_json (uri);
}

DexFuture *
gx_query_flathub_v2_json_take (char *request)
{
  DexFuture *future = NULL;

  dex_return_error_if_fail (request != NULL);

  future = gx_query_flathub_v2_json (request);
  g_free (request);
