...
#define G_LOG_DOMAIN  "GAMERX_STORE::FLATPAK-ENTRY"
...
struct _GxFlatpakEntry
{
  GxEntry parent_instance;
...
static void
serializable_iface_init (GxSerializableInterface *iface);

G_DEFINE_FINAL_TYPE_WITH_CODE (
    GxFlatpakEntry,
    gx_flatpak_entry,
    GX_TYPE_ENTRY,
    G_IMPLEMENT_INTERFACE (GX_TYPE_SERIALIZABLE, serializable_iface_init))
...
static void
clear_entry (GxFlatpakEntry *self);

static void
gx_flatpak_entry_dispose (GObject *object)
{
  GxFlatpakEntry *self = GX_FLATPAK_ENTRY (object);

  clear_entry (self);
  g_clear_object (&self->ref);

  G_OBJECT_CLASS (gx_flatpak_entry_parent_class)->dispose (object);
}

static void
gx_flatpak_entry_get_property (GObject    *object,
                               guint       prop_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
...
static void
gx_flatpak_entry_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
...
static void
gx_flatpak_entry_class_init (GxFlatpakEntryClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_flatpak_entry_set_property;
  object_class->get_property = gx_flatpak_entry_get_property;
  object_class->dispose      = gx_flatpak_entry_dispose;

  props[PROP_INSTANCE] =
      g_param_spec_object (
          "instance",
          NULL, NULL,
          GX_TYPE_FLATPAK_INSTANCE,
          G_PARAM_READABLE);
...
static void
gx_flatpak_entry_init (GxFlatpakEntry *self)
{
}

static void
gx_flatpak_entry_real_serialize (GxSerializable  *serializable,
                                 GVariantBuilder *builder)
{
  GxFlatpakEntry *self = GX_FLATPAK_ENTRY (serializable);
  ...
  gx_entry_serialize (GX_ENTRY (self), builder);
}

static gboolean
gx_flatpak_entry_real_deserialize (GxSerializable *serializable,
                                   GVariant       *import,
                                   GError        **error)
{
  GxFlatpakEntry *self          = GX_FLATPAK_ENTRY (serializable);
  ...
  return gx_entry_deserialize (GX_ENTRY (self), import, error);
}

static void
serializable_iface_init (GxSerializableInterface *iface)
{
  iface->serialize   = gx_flatpak_entry_real_serialize;
  iface->deserialize = gx_flatpak_entry_real_deserialize;
}
...
gx_flatpak_entry_new_for_ref (GxFlatpakInstance *instance,
                              ...
                              GError           **error)
{
  g_autoptr (GxFlatpakEntry) self              = NULL;
  GBytes *bytes                                = NULL;
  ...
  g_return_val_if_fail (GX_IS_FLATPAK_INSTANCE (instance), NULL);
  ...
  self       = g_object_new (GX_TYPE_FLATPAK_ENTRY, NULL);
  self->user = user;
...
#define GET_STRING(member, group_name, key) \
  ...
  if (g_key_file_has_group (key_file, "Application"))
    {
      kinds |= GX_ENTRY_KIND_APPLICATION;
  ...
  if (g_key_file_has_group (key_file, "ExtensionOf"))
    {
      if (!(kinds & GX_ENTRY_KIND_RUNTIME))
        kinds |= GX_ENTRY_KIND_ADDON;
...
#undef GET_STRING
  ...
  //     return NULL;
  //   }
  module_dir = gx_dup_module_dir ();

  self->flatpak_id = flatpak_ref_format_ref (ref);

  id                 = flatpak_ref_get_name (ref);
  unique_id          = gx_flatpak_ref_format_unique (ref, user);
  ...
  if (component != NULL)
    {
      ...
      if (screenshots != NULL)
        {
          screenshot_paintables = g_list_store_new (GX_TYPE_ASYNC_TEXTURE);
                  ...
                  if (url != NULL)
                    {
                      ...
                      texture = gx_async_texture_new_lazy (screenshot_file, cache_file);
      ...
      share_urls = g_list_store_new (GX_TYPE_URL);
      if (kinds & GX_ENTRY_KIND_APPLICATION &&
          g_strcmp0 (remote_name, "flathub") == 0)
        {
          ...
          url = gx_url_new ();
          gx_url_set_name (url, C_ ("Project URL Type", "Flathub Page"));
          gx_url_set_url (url, flathub_url);
          ...
          if (url != NULL)
            {
              ...
              g_autoptr (GxUrl) share_url = NULL;
      ...
      if (releases_arr != NULL)
        {
          version_history = g_list_store_new (GX_TYPE_RELEASE);
          ...
          for (guint i = 0; i < releases_arr->len; i++)
            {
              ...
              if (as_issues != NULL && as_issues->len > 0)
                {
                  issues = g_list_store_new (GX_TYPE_ISSUE);
          ...
          if (select != NULL)
            {
              ...
              texture        = gx_async_texture_new_lazy (source, cache_into);
              icon_paintable = GDK_PAINTABLE (texture);

              if (select_is_local)
                mini_icon = gx_load_mini_icon_sync (unique_id_checksum, select);
            }
        }
...
{
  g_return_val_if_fail (GX_IS_FLATPAK_ENTRY (self), NULL);
...
{
  g_return_val_if_fail (GX_IS_FLATPAK_ENTRY (self), FALSE);
  return self->user;
}
                         ...
                         GError           **error)
{
  ...
  g_return_val_if_fail (GX_IS_FLATPAK_ENTRY (self), FALSE);
  g_return_val_if_fail (GX_IS_FLATPAK_INSTANCE (flatpak), FALSE);

  ref = gx_flatpak_entry_get_ref (self);
  ...
  installation =
      self->user
          ? gx_flatpak_instance_get_user_installation (flatpak)
          : gx_flatpak_instance_get_system_installation (flatpak);
