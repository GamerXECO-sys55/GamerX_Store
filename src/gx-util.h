...
#define GX_RELEASE_DATA(name, unref) \
  if ((unref) != NULL)               \
    g_clear_pointer (&self->name, (unref));

#define GX_RELEASE_UTAG(name, remove) \
  if ((remove) != NULL)               \
    g_clear_handle_id (&self->name, (remove));

/* va args = releases */
#define GX_DEFINE_DATA(name, Name, layout, ...)    \
...
typedef DexFuture GxGuard;
static inline void
gx_guard_destroy (GxGuard *guard)
{
  if (dex_future_is_pending (guard))
    dex_promise_resolve_boolean (DEX_PROMISE (guard), TRUE);
  dex_unref (guard);
}
G_DEFINE_AUTOPTR_CLEANUP_FUNC (GxGuard, gx_guard_destroy);
#define gx_clear_guard(_pp) g_clear_pointer (_pp, gx_guard_destroy)

#define GX_BEGIN_GUARD_WITH_CONTEXT(_guard, _mutex, _gate) \
  G_STMT_START                                             \
  {                                                        \
...
#define GX_BEGIN_GUARD(_guard)                             \
  G_STMT_START                                             \
  {                                                        \
    static GMutex   _mutex = { 0 };                        \
    static GxGuard *_gate  = NULL;                         \
    GX_BEGIN_GUARD_WITH_CONTEXT (_guard, &_mutex, &_gate); \
  }                                                        \
  G_STMT_END
