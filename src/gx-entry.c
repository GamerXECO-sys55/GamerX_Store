...
#define G_LOG_DOMAIN  "GAMERX_STORE::ENTRY"
#define GAMERX_MODULE "entry"
...
G_DEFINE_ABSTRACT_TYPE_WITH_PRIVATE (GxEntry, gx_entry, G_TYPE_OBJECT);
...
static gboolean
maybe_save_paintable (GxEntryPrivate  *priv,
                      const char      *key,
...
GX_DEFINE_DATA (
    ...
    GX_RELEASE_DATA (result, g_object_unref))
static DexFuture *
...
static void
clear_entry (GxEntry *self);

static void
gx_entry_dispose (GObject *object)
{
  GxEntry *self = GX_ENTRY (object);

  clear_entry (self);

  G_OBJECT_CLASS (gx_entry_parent_class)->dispose (object);
}

static void
gx_entry_get_property (GObject    *object,
                       guint       prop_id,
                       GValue     *value,
                       GParamSpec *pspec)
{
  GxEntry        *self = GX_ENTRY (object);
  GxEntryPrivate *priv = gx_entry_get_instance_private (self);

  switch (prop_id)
    {
    case PROP_HOLDING:
      g_value_set_boolean (value, gx_entry_is_holding (self));
      break;
    case PROP_INSTALLED:
    ...
    case PROP_ICON_PAINTABLE:
      ...
      dex_unref (gx_entry_load_mini_icon (self));
      break;
    case PROP_MINI_ICON:
...
static void
gx_entry_set_property (GObject      *object,
                       guint         prop_id,
                       const GValue *value,
                       GParamSpec   *pspec)
{
        ...
        if (priv->download_stats != NULL)
          {
            ...
            for (guint i = start; i < n_items; i++)
              {
                g_autoptr (GxDataPoint) point = NULL;

                point = g_list_model_get_item (priv->download_stats, i);
                recent_downloads += gx_data_point_get_dependent (point);
              }
...
static void
gx_entry_class_init (GxEntryClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->set_property = gx_entry_set_property;
  object_class->get_property = gx_entry_get_property;
  object_class->dispose      = gx_entry_dispose;
...
static void
gx_entry_real_serialize (GxSerializable  *serializable,
                         GVariantBuilder *builder)
{
  GxEntry        *self = GX_ENTRY (serializable);
  GxEntryPrivate *priv = gx_entry_get_instance_private (self);
      ...
      if (n_items > 0)
        {
          ...
          for (guint i = 0; i < n_items; i++)
            {
              g_autoptr (GxUrl) url = NULL;
              const char *name      = NULL;
              const char *url_str   = NULL;

              url     = g_list_model_get_item (priv->share_urls, i);
              name    = gx_url_get_name (url);
              url_str = gx_url_get_url (url);
      ...
      if (n_items > 0)
        {
          ...
          for (guint i = 0; i < n_items; i++)
            {
              g_autoptr (GxRelease) release              = NULL;
              ...
              issues      = gx_release_get_issues (release);
              timestamp   = gx_release_get_timestamp (release);
              url         = gx_release_get_url (release);
              version     = gx_release_get_version (release);
              description = gx_release_get_description (release);
                  ...
                  if (n_issues > 0)
                    {
                      ...
                      for (guint j = 0; j < n_issues; j++)
                        {
                          g_autoptr (GxIssue) issue = NULL;
                          const char *issue_id      = NULL;
                          const char *issue_url     = NULL;

                          issue     = g_list_model_get_item (issues, j);
                          issue_id  = gx_issue_get_id (issue);
                          issue_url = gx_issue_get_url (issue);
                  ...
                  for (guint i = 0; i < n_items; i++)
                    {
                      ...
                      point       = g_list_model_get_item (priv->download_stats, i);
                      independent = gx_data_point_get_independent (point);
                      dependent   = gx_data_point_get_dependent (point);
                      label       = gx_data_point_get_label (point);
...
static gboolean
gx_entry_real_deserialize (GxSerializable *serializable,
                           GVariant       *import,
                           GError        **error)
{
  GxEntry        *self          = GX_ENTRY (serializable);
  ...
  for (;;)
    {
      ...
      if (g_strcmp0 (key, "installed") == 0)
      ...
      else if (g_strcmp0 (key, "screenshot-paintables") == 0)
        {
          ...
          store = g_list_store_new (GX_TYPE_ASYNC_TEXTURE);
      ...
      else if (g_strcmp0 (key, "share-urls") == 0)
        {
          ...
          store = g_list_store_new (GX_TYPE_URL);
          ...
          for (;;)
            {
              ...
              if (!g_variant_iter_next (url_iter, "(ss)", &name, &url_str))
                break;
              url = gx_url_new ();
              gx_url_set_name (url, name);
              gx_url_set_url (url, url_str);
      ...
      else if (g_strcmp0 (key, "version-history") == 0)
        {
          ...
          store = g_list_store_new (GX_TYPE_RELEASE);
              ...
              if (issues != NULL)
                {
                  ...
                  issues_store = g_list_store_new (GX_TYPE_ISSUE);
                  ...
                  for (;;)
                    {
                      ...
                      issue = gx_issue_new ();
                      gx_issue_set_id (issue, issue_id);
                      gx_issue_set_url (issue, issue_url);
                      g_list_store_append (issues_store, issue);
                    }
                }

              release = gx_release_new ();
              if (issues_store != NULL)
                gx_release_set_issues (release, G_LIST_MODEL (issues_store));
              gx_release_set_timestamp (release, timestamp);
              gx_release_set_url (release, url);
              gx_release_set_version (release, version);
              gx_release_set_description (release, description);
      ...
      //     store = g_list_store_new (GX_TYPE_DATA_POINT);
      ...
      //           break;
      //         point = gx_data_point_new ();
      //         gx_data_point_set_independent (point, independent);
      //         gx_data_point_set_dependent (point, dependent);
      //         gx_data_point_set_label (point, label);
...
{
  GxEntryPrivate *priv = NULL;

  g_return_if_fail (GX_IS_ENTRY (self));
  priv = gx_entry_get_instance_private (self);
...
gx_entry_is_holding (GxEntry *self)
{
  ...
  g_return_val_if_fail (GX_IS_ENTRY (self), FALSE);
  priv = gx_entry_get_instance_private (self);
...
gx_entry_get_addons (GxEntry *self)
{
  ...
  g_return_val_if_fail (GX_IS_ENTRY (self), NULL);
  priv = gx_entry_get_instance_private (self);
...
gx_entry_get_eol (GxEntry *self)
{
  ...
  g_return_val_if_fail (GX_IS_ENTRY (self), 0);
  priv = gx_entry_get_instance_private (self);
...
gx_entry_load_mini_icon (GxEntry *self)
{
  ...
  dex_return_error_if_fail (GX_IS_ENTRY (self));
  priv = gx_entry_get_instance_private (self);

  if (priv->mini_icon == NULL &&
      priv->mini_icon_future == NULL &&
      GX_IS_ASYNC_TEXTURE (priv->icon_paintable))
    {
      dex_clear (&priv->mini_icon_future);
      priv->mini_icon_future = dex_future_then (
          gx_async_texture_dup_future (GX_ASYNC_TEXTURE (priv->icon_paintable)),
                    ...
                    GVariantBuilder *builder)
{
  ...
  return gx_entry_real_serialize (GX_SERIALIZABLE (self), builder);
}
                      ...
                      GError  **error)
{
  ...
  return gx_entry_real_deserialize (GX_SERIALIZABLE (self), import, error);
}
...
query_flathub_fiber (QueryFlathubData *data)
{
  ...
  node = dex_await_boxed (gx_query_flathub_v2_json (request), &local_error);
  if (node == NULL)
    {
                      ...
                      GVariantBuilder *builder)
{
  ...
  if (!GX_IS_ASYNC_TEXTURE (paintable))
    {
      g_warning ("Paintable must be of type GxAsyncTexture to be serialized!");
      return FALSE;
    }

  source_uri      = gx_async_texture_get_source_uri (GX_ASYNC_TEXTURE (paintable));
  cache_into_path = gx_async_texture_get_cache_into_path (GX_ASYNC_TEXTURE (paintable));
  if (cache_into_path == NULL)
    goto done;

  if (gx_async_texture_get_loaded (GX_ASYNC_TEXTURE (paintable)))
    texture = gx_async_texture_dup_texture (GX_ASYNC_TEXTURE (paintable));
  else
    goto done;
...
make_async_texture (GVariant *parse)
{
  ...
  texture = gx_async_texture_new_lazy (source_file, cache_into_file);
                            ...
                            GxEntry   *self)
{
  ...
  /* ? */
  if (!GX_IS_ASYNC_TEXTURE (priv->icon_paintable))
    return NULL;

  icon_path = gx_async_texture_get_cache_into_path (GX_ASYNC_TEXTURE (priv->icon_paintable));
  if (icon_path == NULL)
    return NULL;
...
load_mini_icon_fiber (LoadMiniIconData *data)
{
  ...
  data->result = load_mini_icon_sync (
      gx_entry_get_unique_id_checksum (GX_ENTRY (self)),
      path);
  return dex_scheduler_spawn (
