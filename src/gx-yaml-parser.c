...
/* clang-format off */
G_DEFINE_QUARK (gx-yaml-error-quark, gx_yaml_error);
/* clang-format on */
...
G_DEFINE_FINAL_TYPE (GxYamlParser, gx_yaml_parser, G_TYPE_OBJECT)
...
static void
gx_yaml_parser_dispose (GObject *object)
{
  GxYamlParser *self = GX_YAML_PARSER (object);

  g_clear_pointer (&self->schema, schema_node_data_unref);

  G_OBJECT_CLASS (gx_yaml_parser_parent_class)->dispose (object);
}

static void
gx_yaml_parser_class_init (GxYamlParserClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = gx_yaml_parser_dispose;
}

static void
gx_yaml_parser_init (GxYamlParser *self)
{
}
...
gx_yaml_parser_new_for_resource_schema (const char *path)
{
  ...
  parser         = g_object_new (GX_TYPE_YAML_PARSER, NULL);
                              ...
                              GError      **error)
{
  ...
  g_return_val_if_fail (GX_IS_YAML_PARSER (self), NULL);
